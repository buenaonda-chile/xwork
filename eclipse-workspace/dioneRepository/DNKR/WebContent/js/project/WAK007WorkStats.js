...
//					teamCode에 리스트 초기화
					$('select.empNumber').html('<option>성명</option>');
				}
...
$(function() {
	...
	var workStatsCalendar = new ax5.ui.calendar({
        ...
        onClick: function () {
            ...
            console.log(workStatsCalendar.getSelection());
...
//            console.log(this);
	...
	$('._searchBtn_').on('click', function(e){
		...
		if(val_chk(divEl.find('#endDate').val(), '마지막 날짜')
...
function downloadExcel(workType){
	//fnExcelReport('pjtExcel', 'pjtInfo');
	location.href = "/WAK007WorkStatsSearch/workStatExcel.do?filename=pjtInfo&workType="+workType+"&startDate="+$('#startDate').val()+"&endDate="+$('#endDate').val();
}