...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/dwh/WAE040JdeBudget.js"></script>
<!-- [EN] Contents area -->
			...
			<div class="top-bar cb">
				...
				<div class="date-inwrap" data-ax5picker="basic" style="width: 120px;">
					<span class="calendar_icon_b vcen2 c1"></span> <input type="text"
						...
						placeholder="yyyy" data-picker-date="date"
						value='<fmt:formatDate value="${today}" pattern="yyyy"/>'  onchange="commonClear()"/>
				</div>
				
				<!-- 전표구분을 선택해주세요 -->
				&nbsp;&nbsp;&nbsp; 
				<strong class="pr5">부서코드</strong>
				<select class="pDpcode" autofocus="true" name="pDpcode"  id="pDpcode" style="width:180px" onchange="commonClear()">
					<c:if test="${fn:length(codeMst650PFList) > 0}">
						<option value="0" selected><c:out value="전체"/></option>
						<c:forEach var="codeMst650Pf" items="${codeMst650PFList}" varStatus="status">
							<option value="${codeMst650Pf.dpCode}">
							<c:out value="${codeMst650Pf.dpCoDpnm}"/></option>
						</c:forEach>
					</c:if>
				</select>
				<!-- 계정과목을 선택해주세요 -->
				&nbsp;&nbsp;&nbsp; 
				<strong class="pr5">BU코드</strong>
				<select class="pGbmcu" autofocus="true" name="pGbmcu"  id="pGbmcu" style="width:180px" onchange="commonClear()">
					<c:if test="${fn:length(codeF0006LBList) > 0}">
						<option value="0" selected><c:out value="전체"/></option>
						<c:forEach var="codeF0006LB" items="${codeF0006LBList}" varStatus="status">
							<option value="${codeF0006LB.mcmcu}">
							<c:out value="${codeF0006LB.mcName}"/></option>
						</c:forEach>
					</c:if>
				</select>
				<!-- 자료선택 -->
				<strong class="pr5"><spring:message code='label.selectMenu'/><%-- 자료선택--%></label></strong>
					<input type="radio" name="selectMenu" value="BUCODE" onclick="commonClear()" checked="checked">BU코드정렬
					<input type="radio" name="selectMenu" value="DPCODE" onclick="commonClear()"> 부서정렬
				<!-- 조회년도를 선택해주세요 -->
				<%-- 
				~
				<select class="pGbmcu2" autofocus="true" name="pGbmcu2"  id="pGbmcu2" contenteditable="true" style="width:180px" onchange="commonClear()">
					<c:if test="${fn:length(codeF0006LBList) > 0}">
