 ...
 */
@Controller
@RequestMapping(value = "/WAE013OceanListSearch")
@ViewId(value = "WAE013")
public class WAE013OceanListController {
	 ...
	 * [JP] デフォルトコンストラクタ。<br>
	 */
	public WAE013OceanListController() {
		// no process
	}
	...
	public ModelAndView initOceanListController(HttpServletRequest request,
			@ModelAttribute("WAE013OceanListForm") WAE013OceanListForm form)
			throws Exception {
		/**
		 * <pre>
		 *             
		...
		mv.addObject("url", "WAE013OceanList.jsp");
		mv.setViewName(".tiles.page.WAE013OceanList");
		return mv;
	}
	...
	public ModelAndView oceansListSearch(HttpServletRequest request,
		...
		String year="";
		List<OceanListDomain> oceansList = null;
				
		if(dwhDomain2 != null){
			year =dwhDomain2.getJyymm();
			jsonObj.put("year", year);
			try {
				oceansList = mifService.searchByOceansList(form);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (oceansList != null) {
