...
import com.globaldenso.dnkr.presentation.form.dwh.WAE028FakeSalesCheckListgForm;
 ...
 */
@Controller
@RequestMapping(value = "/WAE028FakeSalesCheckListSearch")
@ViewId(value = "WAE028")
public class WAE028FakeSalesCheckListController {
	 ...
	 * [JP] デフォルトコンストラクタ。<br>
	 */
	public WAE028FakeSalesCheckListController() {
		// no process
	}
	...
	public ModelAndView initFakeSalesCheckList(HttpServletRequest request,
		...
		mv.addObject("url", "WAE028FakeSalesCheckList.jsp");
		mv.setViewName(".tiles.page.WAE028FakeSalesCheckList");
		return mv;
	}
	...
	public ModelAndView fakeSalesCheckListSearch(HttpServletRequest request,
			@ModelAttribute("WAE028FakeSalesCheckListgForm") WAE028FakeSalesCheckListgForm form)
			throws Exception {
		...
		String year="";
		List<FakeSalesCheckListDomain> fakeSalesCheckList = null;
		
		if(dwhDomain2 != null){
			year =dwhDomain2.getJyymm();
			jsonObj.put("year", year);
			try {
				fakeSalesCheckList = mifService.searchByFakeSalesCheckList(form);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (fakeSalesCheckList != null) {
				result = DnkrConstans.RESULT_Y;
				jsonObj.put("list", fakeSalesCheckList);
			}
		}
