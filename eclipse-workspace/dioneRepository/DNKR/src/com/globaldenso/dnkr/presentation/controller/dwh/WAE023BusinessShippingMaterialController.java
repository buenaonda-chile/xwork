...
import com.globaldenso.dnkr.presentation.form.dwh.WAE023BusinessShippingMaterialForm;
import com.globaldenso.dnkr.uxis.UxisCmmUtil;

/**
 * [KO] [종합관리]-[판매정보]-영업출하현황(재료비) Controller.<br>
 * [JP] <br>
 * 
 * @author $Author$
 * @version $Revision$
 */
@Controller
@RequestMapping(value = "/WAE023BusinessShippingMaterialSearch")
@ViewId(value = "WAE023")
public class WAE023BusinessShippingMaterialController {
	 ...
	 * [JP] デフォルトコンストラクタ。<br>
	 */
	public WAE023BusinessShippingMaterialController() {
		// no process
	}

	/**
	 * [종합관리] - [판매정보] - 영업출하현황(재료비)  초기화면[KR] <br>
	 * 
	...
	public ModelAndView initBusinessShipping(HttpServletRequest request,
		...
		mv.addObject("url", "WAE023BusinessShippingMaterial.jsp");
		mv.setViewName(".tiles.page.WAE023BusinessShippingMaterial");
		return mv;
	}

	/**
	 * [종합관리] - [판매정보] - 영업출하현황(재료비) 데이터 조회
	 * 
	 * @param request HttpServletRequest
	 * @param form    WAE024BusinessShippingForm
	 * @return ModelAndView
	...
	public ModelAndView businessShippingSearch(HttpServletRequest request,
			@ModelAttribute("WAE023BusinessShippingMaterialForm") WAE023BusinessShippingMaterialForm form)
			throws Exception {
		...
		String year="";
		List<BusinessShippingMaterialDomain> businessShippingMaterialList = null;
		
		if(dwhDomain2 != null){
			year =dwhDomain2.getJyymm();
			jsonObj.put("year", year);
			try {
				businessShippingMaterialList = mifService.searchByBusinessShippingMaterial(form);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (businessShippingMaterialList != null) {
				result = DnkrConstans.RESULT_Y;
				jsonObj.put("list", businessShippingMaterialList);
			}
		}
