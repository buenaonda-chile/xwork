 ...
 */
@Controller
@RequestMapping(value = "/WAE002PurchasePriceSearch")
@ViewId(value = "WAE002")
public class WAE002PurchasePriceSearchController {
	 ...
	 * [JP] デフォルトコンストラクタ。<br>
	 */
	public WAE002PurchasePriceSearchController() {
		// no process
	}
	...
	public ModelAndView initPurchasePrice(HttpServletRequest request,
			@ModelAttribute("WAE002PurchasePriceForm") WAE002PurchasePriceForm form)
			throws Exception {
		/**
		 * <pre>
		 *             
		...
		mv.addObject("url", "WAE002PurchasePrice.jsp");
		mv.setViewName(".tiles.page.WAE002PurchasePrice");
		return mv;
	}
	...
	public ModelAndView materialReceivingSearch(HttpServletRequest request,
		...
		List<PurchasePriceDomain> purchasePriceList = null;
		try {
			purchasePriceList = mifService.searchByPurchasePrice(form);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if (purchasePriceList != null ) {
			...
			jsonObj.put("list", purchasePriceList);
		}
