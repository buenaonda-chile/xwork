...
import com.globaldenso.dnkr.presentation.form.dwh.WAE034ProductShippingReceivingForm;
 ...
 */
@Controller
@RequestMapping(value = "/WAE034ProductShippingReceivingSearch")
@ViewId(value = "WAE034")
public class WAE034ProductShippingReceivingController {
	 ...
	 * [JP] デフォルトコンストラクタ。<br>
	 */
	public WAE034ProductShippingReceivingController() {
		// no process
	}
	...
	public ModelAndView initProductShippingReceiving(HttpServletRequest request,
		...
		mv.addObject("url", "WAE034ProductShippingReceiving.jsp");
		mv.setViewName(".tiles.page.WAE034ProductShippingReceiving");
		return mv;
	}
	...
	public ModelAndView productShippingReceivingSearch(HttpServletRequest request,
			@ModelAttribute("WAE034ProductShippingReceivingForm") WAE034ProductShippingReceivingForm form)
			throws Exception {
		...
		String year="";
		List<ProductShippingReceivingDomain> productShippingReceivingList = null;
		
		if(dwhDomain2 != null){
			year =dwhDomain2.getJyymm();
			jsonObj.put("year", year);
			try {
				productShippingReceivingList = mifService.searchByProductShippingReceiving(form);
			} catch (Exception e) {
				e.printStackTrace();
			}
