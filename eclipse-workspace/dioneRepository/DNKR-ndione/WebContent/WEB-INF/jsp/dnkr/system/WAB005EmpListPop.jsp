...
<h2 class="subpage-tit"><c:out value="${workTypeNm}"/> <spring:message code='label.circulation'/>&nbsp;<spring:message code='label.select'/><!-- 회람자선택 --></h2>
<div class="board-bList-wrap">
	<div class="list-top">
		<div class="list-top text-right">
			<form:form action="${pageContext.request.contextPath}/WAB001EmpSearch/empListPop.do" method="POST" name="EmpDomain" id="WAB005EmpList">
		<div class="list-top text-right">
						...
						<c:if test="${fn:length(empList) > 0}">
							...
							<c:forEach var="empDomain" items="${empList}" varStatus="status">
								<tr>
									...
									<!-- 팀네임에 특수문자가 들어가있을경우 제거해서 네임값으로 넣음 -->
									<!-- 
										<c:set var="teamName" value="${fn:replace(empDomain.teamName,' ','_') }"/>
										<c:set var="teamName" value="${fn:replace(teamName,'(','_') }"/>
										<c:set var="teamName" value="${fn:replace(teamName,')','_') }"/>
										<c:set var="teamName" value="${fn:replace(teamName,'.','_') }"/>
										<c:set var="teamName" value="${fn:replace(teamName,'/','_') }"/>
										 -->
									...
									<td><a href="#" onclick='teamClick("${empDomain.teamCode}");'><c:out value="${empDomain.teamName}"/></a></td><!-- 부서(팀) -->
									<td><c:out value="${empDomain.empName}"/> ( <c:out value="${empDomain.empId}"/> )</td><!-- 성명 -->
									<td><!-- 체크박스 -->
										
										<input type="checkbox" id="empNumList[${status.index}]" name="empNumList_${empDomain.teamCode}" class="empNumList"  value="${empDomain.empName}/${empDomain.empId}/${empDomain.teamCode}"
											<%--<c:if test="${workCode eq aprvEmpMap[empDomain.empNumber]}">
												checked
											</c:if> --%>
											<%--value="${empDomain.empName}/${empDomain.empId}/${empDomain.teamCode}" --%>
										/>
									</td>
								</tr>
							</c:forEach>
