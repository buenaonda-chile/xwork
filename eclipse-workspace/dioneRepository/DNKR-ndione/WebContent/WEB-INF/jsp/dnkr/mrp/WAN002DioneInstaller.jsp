...
<SCRIPT LANGUAGE="JavaScript">
	...
	function fnCheckModule(){
		...
		try	{
			if(MiInstaller.Version == undefined){ //설치가 안되어 있거나 system32에 MiInstlr321.dll 이 없는 경우.(unregistered 된 경우)
				//alert("false");
	...
	function fnOnLoad() {
		...
		if(rltCheck=="Installed") {
			...
			if(Bcnt) {
				...
				//fnRun();// 체크픞래그 추가 
				// 20181016 
				// 추가함 END Page() session 플레그 추가
				var rlt = setSessionSuccess();
				//alert("SUCCESS 1111" + rlt);
				$('#DioneInstallerForm').submit();
			}
		}
...
<form:form id="DioneInstallerForm" name ="DioneInstallerForm" action="${pageContext.request.contextPath}/WAN001MiPlatform/view.do?tabId=${tabId}&menu=${menu}&pgmId=${pgmID}&company=${company}&mode=${mode}">
