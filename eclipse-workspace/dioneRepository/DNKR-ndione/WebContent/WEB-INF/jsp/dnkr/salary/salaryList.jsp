...
$(document).ready(function(){
	...
	$('#jqGridList').jqGrid({ 
	    ...
	    colNames : ['SEQ', '연봉제목', '체결기간',
	    	'<spring:message code="label.edulist8"/>', '<spring:message code="label.edulist9"/>', '아이디', '<spring:message code="label.edulist10"/>'],
	    colModel : [
					{name:'pk',index:'pk', width:55, hidden : true},
					{name:'subject',index:'subject', width:200, excel:'아이디'},
					{name:'start_date',index:'start_date', width:200, align:'center'},
					{name:'mod_date',index:'mod_date', align:'center', width:100},
					{name:'mod_user',index:'mod_user', width:100, sortable:false, align:'center'},
					{name:'reg_user_id',index:'reg_user_id', width:60, sortable:false, align:'center'},
		...
		gridComplete : function() {
			...
			for ( var i = 0; i < rows.length; i++ ) {
				...
				var reg_user_id = $('#jqGridList').getCell( rows[i], "reg_user_id" );
				
				if($("#emp_number").val() != reg_user_id) {
		...
		onCellSelect : function(row_id, iCol) {
			...
			if(row_id != null) {
				...
				if($("#emp_number").val() == item.reg_user_id) {
					location.href = "salaryDetail.do?edu_pk=" + item.pk;
				}
	        }
		}
	}); //end of jqGrid
...
<body>
    ...
    <input type="hidden" id="user_name" name="user_name" value="${user_name }" />
        ...
        <div class="doc-wrap" id="docBaseWrap">
            <div class="layout-LW">
                <!-- 좌측 -->
                <div class="left" style="width:1300px;">
                    <!-- 1번표 -->
