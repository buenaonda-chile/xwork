...
function validation(item) {
	...
	if (item.endcnm == null || wijmo.isEmpty(item.endcnm) || item.endcnm == "") {
		alert("수정이나 추가한 행의 ["+LB_LAST_CSTMR_NAME+"]컬럼을 입력해야 됩니다.");
		return false;
	}
...
function gridInit() {
	var columnsDefinition = [
		...
		{binding: 'endcnm', header: '* ' + LB_LAST_CSTMR_NAME, width: '*', align: 'left', dataType: 'String', maxLength: 50}, //최공고객명
	...
	cv.getError = function(item, prop) {
		switch (prop) {
			...
			case "endcnm":
				...
				if (item.endcnm == null || wijmo.isEmpty(item.endcnm) || item.endcnm == "") {
					return "["+LB_LAST_CSTMR_NAME+"]를 입력해야 됩니다.";
				}
				return null;
