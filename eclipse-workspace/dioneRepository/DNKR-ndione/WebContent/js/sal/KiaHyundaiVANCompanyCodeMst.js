...
function search() {
	...
	var url = '/ajaxKiaHyundaiVANCompanyCodeMst/search.do';
	var params = {
		'getComps' : $(".getComps").val(),
...
function validation(item){
	...
	if(item.vndcd==null || wijmo.isEmpty(item.vndcd) || item.vndcd == ""){
		alert("수정이나 추가한 행의 [고객사코드]컬럼을 입력하세요");
		return false;
	}
	
	if(item.faccd==null || wijmo.isEmpty(item.faccd) || item.faccd ==""){
		alert("수정이나 추가한 행의 [공장코드]컬럼을 입력하세요");
		return false;
	}
	
	if(item.facnm==null || wijmo.isEmpty(item.facnm) || item.facnm ==""){
		alert("수정이나 추가한 행의 [공장명]컬럼을 입력하세요");
		return false;
	}
...
function update(){
    ...
    for ( var i = 0; i <cv.itemsEdited.length; i ++) {
		...
		$.ajax({
			method : "post",
			url : "/ajaxKiaHyundaiVANCompanyCodeMst/update.do",
    ...
    for ( var i = 0; i <cv.itemsAdded.length; i ++) {
		...
		$.ajax({
			method : "post",
			url : "/ajaxKiaHyundaiVANCompanyCodeMst/create.do",
    ...
    for ( var i = 0; i <cv.itemsRemoved.length; i ++) {
		...
		$.ajax({
			method : "post",
			url : "/ajaxKiaHyundaiVANCompanyCodeMst/delete.do",
...
function downloadExcel(){
	...
	var fileName = '기아현대 VAN 업체코드마스타'+".xlsx";
...
function handleEvent(){
	...
	$("._addBtn_").on("click",function(){
		...
		// 사업부코드, 업체코드 조회한 값 그대로 셋팅
		addRow.pscom  = $(".getComps").val();
		addRow.psvnd1 = $(".getPyvnd").val();
		cv.commitNew();
	...
	$('.getComps').on('change', function() {
		...
		if(getComps == "E1"){
			...
			//컬럼정의
			columnsDefinition = [
				{binding:'vndcd',visible:false},
			    {binding:'faccd',visible:false},
			    {binding:'facnm',visible:false},
			    {binding:'pscom',visible:false},
			    {binding:'psvnd1',visible:false},
			    {binding:'oldVndcd',visible:false},
			    {binding:'oldFaccd',visible:false},
			    {binding:'oldFacnm',visible:false},
			    {binding:'oldPscom',visible:false},
			    {binding:'oldPsvnd1',visible:false},
		        {binding: 'vndcd', header: '* '+ LN_CHK_VNDCD, width: '*', wordWrap: true, align:'center',dataType:'String',maxLength:4 , isReadOnly: true },  //wordWrap: true: 텍스트 자동줄바꿈 autorowHeights랑 같이 사용
		        {binding: 'faccd', header: '* '+ LN_CHK_FACCD, width: '*', align:'center',dataType:'String', format:'n0', maxLength:4 },
		        {binding: 'facnm', header: '* '+ LN_CHK_FACNM, width: '*', width: '*', align:'center',dataType:'String', maxLength:20},
		        {binding: 'pscom', header: '* '+ LN_CHK_PSCOM, width: '*', width: '*', align:'center', isReadOnly: true },
		        {binding: 'psvnd1', header: '* '+ LN_CHK_PSVND1, width: '*', width: '*', align:'center', isReadOnly: true}
				// 콤보박스 추가하려면 dataMap
		    ];
		...
		// 마산외(창원,화성) : 서열SEQ
			...
			columnsDefinition = [
			    ...
			    {binding:'oldPsvnd1',visible:false},
		        {binding: 'vndcd', header: '* '+ LN_CHK_VNDCD, width: '*', wordWrap: true, align:'center',dataType:'String',maxLength:4 , isReadOnly: true },  //wordWrap: true: 텍스트 자동줄바꿈 autorowHeights랑 같이 사용
		        {binding: 'faccd', header: '* '+ LN_CHK_SEQNO, width: '*', align:'center',dataType:'String', format:'n0', maxLength:4 },
		        {binding: 'facnm', header: '* '+ LN_CHK_FACNM, width: '*', width: '*', align:'center',dataType:'String', maxLength:20},
		        {binding: 'pscom', header: '* '+ LN_CHK_PSCOM, width: '*', width: '*', align:'center', isReadOnly: true },
		        {binding: 'psvnd1', header: '* '+ LN_CHK_PSVND1, width: '*', width: '*', align:'center', isReadOnly: true}
				// 콤보박스 추가하려면 dataMap
	...
	cv.getError = function(item,prop){
		switch(prop){
			case "vndcd":
				...
				if(item.vndcd==null || wijmo.isEmpty(item.vndcd) || item.vndcd == ""){
					return "[고객사코드]를 입력 하세요.";
				}
				return null;
			case "faccd":
				
				if(item.faccd==null || wijmo.isEmpty(item.faccd) || item.faccd == ""){
					return "[공장코드]를 입력 하세요.";
				}
				return null;
			case "facnm":
