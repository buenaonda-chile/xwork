...
import com.globaldenso.dnkr.presentation.form.dwh.WAE026MonthlySalesPlanForm;
import com.globaldenso.dnkr.uxis.UxisCmmUtil;

/**
 * [KO] [종합관리]-[판매정보]-월판매계획현황 Controller.<br>
 * [JP] <br>
 * 
 * @author $Author$
 * @version $Revision$
 */
@Controller
@RequestMapping(value = "/WAE026MonthlySalesPlanSearch")
@ViewId(value = "WAE026")
public class WAE026MonthlySalesPlanController {
	 ...
	 * [JP] デフォルトコンストラクタ。<br>
	 */
	public WAE026MonthlySalesPlanController() {
		// no process
	}

	/**
	 * [종합관리] - [판매정보] - 월판매계획현황  초기화면[KR] <br>
	 * 
	...
	public ModelAndView initMonthlySalesPlan(HttpServletRequest request,
		...
		mv.addObject("url", "WAE026MonthlySalesPlan.jsp");
		mv.setViewName(".tiles.page.WAE026MonthlySalesPlan");
		return mv;
	}

	/**
	 * [종합관리] - [판매정보] - 월판매계획현황 데이터 조회
	 * 
	 * @param request HttpServletRequest
	 * @param form    WAE026MonthSalesPlanForm
	 * @return ModelAndView
	...
	public ModelAndView monthlySalesPlanSearch(HttpServletRequest request,
			@ModelAttribute("WAE026MonthlySalesPlanForm") WAE026MonthlySalesPlanForm form)
			throws Exception {
		...
		String year="";
		List<MonthlySalesPlanDomain> monthSalesPlanList = null;
		
		if(dwhDomain2 != null){
			year =dwhDomain2.getJyymm();
			jsonObj.put("year", year);
			try {
				monthSalesPlanList = mifService.searchByMonthlySalesPlan(form);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (monthSalesPlanList != null) {
				result = DnkrConstans.RESULT_Y;
				jsonObj.put("list", monthSalesPlanList);
			}
		}
