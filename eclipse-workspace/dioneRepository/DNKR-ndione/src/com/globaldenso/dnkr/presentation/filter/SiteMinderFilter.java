        ...
        try {
            if("I".equals(inOutType)){
				...
				if(rtnFlag.equals("Y")){
			        ...
			        System.out.println("EmpNo = "+criteria.getEmpNumber());
					//System.out.println("criteria==="+criteria);
                    empDomain = userInfoService.searchById(criteria);	///GCI전환으로 로직변경필요 
...
//                    System.out.println("데이터를 받아오지 못했습니다.");
            ...
            if(ExcMsg.contains("gci.denso.com:389")){
            	//System.out.println("connect timed out");
            	request.getSession().setAttribute(ATTRIB_KEY_LDAP_CHECK, "G");	//LDAP 상태체크 - 2020.04.17(JHB)
            }else {
                request.getSession().setAttribute(ATTRIB_KEY_LDAP_CHECK, "N");	//LDAP 상태체크 - 2020.04.17(JHB)            	
            }
            //e.printStackTrace();
        }
...
//        String pass = "password";
    ...
    private void renderLoginPage(
        ...
        if (0 < tryNo) {
            ...
            /*
            String resultSt = "N";
            if(request.getSession().getAttribute("LDAP_CHECK") != null) {
            	resultSt = request.getSession().getAttribute("LDAP_CHECK").toString();
            }
            response.addCookie(new Cookie(ATTRIB_KEY_LDAP_CHECK,  resultSt));	//LDAP 상태체크 - 2020.04.17(JHB)  
            */    
        }
...
//              uid, teamCode, teamName, authCode, empName, empPrivateAuth);
    ...
    public String checkLoginLdap(HttpServletRequest request, EmpDomain emp) throws Exception {
        ...
        if("40004".equals(ldapDomainCheck)) {
        	...
        	System.out.println("JP AD : ldapUser = " + emp.getEmpId() + " = "+InetAddress.getByName("gci.denso.com"));
        	ldapHost = ldapHostJp;
        	domain = domainJp;
        	...
        	System.out.println("KR AD : ldapUser = " + emp.getEmpId());
        	ldapHost = ldapHostKr;
        	domain = domainKr;
        ...
        try {
            ...
            if(("").equals(searchCn.trim())){   //계정이 없는 경우
                resultMessage = "Window AD 등록되지 않은 사용자입니다."; 
                resultFlag= "A"; //(ID Wrong)
                if(("gci.denso.com").equals(domain)) {
                	 resultMessage = "아이디 또는 비번이 맞지 않습니다.";
                     resultFlag= "N"; //(PW Wrong)
                }
            }else{
                ...
                }else if(("52e").equals(ldapErrorKey)){   //계정이 잠긴 경우
                    //if(!("0").equals(searchLockoutTime)){   //계정이 잠긴 경우
                	  resultMessage = "잘못된 Password입니다.";
                      resultFlag= "C"; //(PW Wrong)
                }else{
                    resultMessage = "LDAP 인증오류 입니다.";
                    resultFlag= "D"; //(PW Wrong)
