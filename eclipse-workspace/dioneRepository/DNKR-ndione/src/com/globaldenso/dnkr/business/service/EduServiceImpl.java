...
public class EduServiceImpl implements EduService {
	...
	public Map<String, String> eduSave(CommandMap commandMap, MultipartHttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (commandMap.get("edu_pk").equals(""))
		{
	        ...
	        //System.out.println("eduPk : " + eduPk);
	        ...
	        if (eduPk == null || eduPk.equals("")) {
	        	// 새로운 교육 PK취득
				int newEduPk = eduDao.getEduPk(commandMap.getMap());
				commandMap.put("edu_pk", newEduPk);
	        } else {
			...
			// 추가된 팀코드로 사번 취득
			List<Map<String, Object>> empNumberList = eduDao.getEmpNumber(tempMap);
		...
		else
		{
			...
			// DB등록되어 있는 데이터 취득
			Map<String, Object> getEdu = getEdu(commandMap);
			// DB팀코드 
			String dbDept = (String) getEdu.get("DEPT");
			...
			// DB 사번 취득
			String dbEmpNumber = (String) getEdu.get("EMP_NUMBER");
...
//				empDomain.setCmcUseYN(String.valueOf(commandMap.get("p_emp_type_step")));
	...
	public List<Map<String, Object>> getFileDataList(String edu_pk) throws ApplicationException {
		return eduDao.getFileDataList(edu_pk);
	}
	 ...
	 */
    @Override
    public Map<String, Object> saveAttachFile(MultipartFile file, MultipartHttpServletRequest request, CommandMap commandMap) throws Exception {
		...
		if(size > 0) {
	        ...
	        String eduPk =  (String) request.getParameter("edu_pk");
	        //System.out.println("eduPk : " + eduPk);
	        ...
	        if (eduPk == null || eduPk.equals("")) {
	            ...
	            //System.out.println("newEduPk : " + newEduPk);
	            
	            map.put("edu_pk", newEduPk);
	        } else {
	        	map.put("edu_pk", eduPk);
	        }
			...
			int attachInsertResult = eduDao.eduAttchInsert(map);
    ...
    // 첨부파일 1건 삭제
    @Override
    public Map<String, String> eduAttachDel(CommandMap commandMap) throws Exception {
        // 결과값 최초
        String result = Constants.RESULT_FAIL;

        // 1건삭제
        int saveResult = eduDao.eduAttachDel(commandMap.getMap());
        if (saveResult > 0)
        {
            result = Constants.RESULT_SUCCESS;
        }
	...
	public Map<String, String> eduDel(CommandMap commandMap) throws ApplicationException, Exception {
		...
		else
		{
			...
			int attachDelResult = eduDao.eduAttachAllDel(commandMap.getMap());
	...
	public List<Map<String, Object>> getNoEduMailList(String edu_pk) throws ApplicationException {
		...
		return eduDao.getNoEduMailList(edu_pk);
	}
