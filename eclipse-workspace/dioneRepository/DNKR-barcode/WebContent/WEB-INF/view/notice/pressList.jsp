...
<script type="text/javascript">
	function inquerySubmit(page) {
		...
		if(page){
			$('#pressListForm input[name="page"]').val(page-1);
		} else{
			$('#pressListForm input[name="page"]').val(0);
		}
		$('#pressListForm').submit();
	}
	
	function goList(){
		location.href='<c:url value="/notice/press/proc?type=regist"/>';
	}
</script>
<form id="pressListForm" method="get">
				...
				<c:when test="${fn:length(noticePage.content) > 0}">
					<c:forEach var="result" items="${noticePage.content}" varStatus="status">
						<tr>
							...
							<td>
								<a href="<c:url value='/notice/press/detail'/>?seq=${result.seq}&type=update"><c:out value="${result.title}" /></a>
							</td>
							<td>
								관리자
							</td>
