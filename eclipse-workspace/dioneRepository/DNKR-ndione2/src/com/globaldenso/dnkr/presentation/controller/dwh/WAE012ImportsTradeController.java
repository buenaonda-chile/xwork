 ...
 */
@Controller
@RequestMapping(value = "/WAE012ImportsTradeSearch")
@ViewId(value = "WAE012")
public class WAE012ImportsTradeController {
	 ...
	 * [JP] デフォルトコンストラクタ。<br>
	 */
	public WAE012ImportsTradeController() {
		// no process
	}
	...
	public ModelAndView initImportsTradeController(HttpServletRequest request,
			@ModelAttribute("WAE012ImportsTradeForm") WAE012ImportsTradeForm form)
			throws Exception {
		/**
		 * <pre>
		 *             
		...
		mv.addObject("url", "WAE012ImportsTrade.jsp");
		mv.setViewName(".tiles.page.WAE012ImportsTrade");
		return mv;
	}
	...
	public ModelAndView importsTradeSearch(HttpServletRequest request,
		...
		String year="";
		List<ImportsTradeDomain> importsTradeList = null;
				
		if(dwhDomain2 != null){
			year =dwhDomain2.getJyymm();
			jsonObj.put("year", year);
			try {
				importsTradeList = mifService.searchByImportsTrade(form);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (importsTradeList != null) {
				result = DnkrConstans.RESULT_Y;
				jsonObj.put("list", importsTradeList);
			}
		}
