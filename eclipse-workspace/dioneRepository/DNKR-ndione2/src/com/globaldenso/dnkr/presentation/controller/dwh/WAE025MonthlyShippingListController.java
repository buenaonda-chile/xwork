...
import com.globaldenso.dnkr.presentation.form.dwh.WAE025MonthlyShippingListForm;
import com.globaldenso.dnkr.uxis.UxisCmmUtil;

/**
 * [KO] [종합관리]-[판매정보]-월출하지시현황 Controller.<br>
 * [JP] <br>
 * 
 * @author $Author$
 * @version $Revision$
 */
@Controller
@RequestMapping(value = "/WAE025MonthlyShippingListSearch")
@ViewId(value = "WAE025")
public class WAE025MonthlyShippingListController {
	 ...
	 * [JP] デフォルトコンストラクタ。<br>
	 */
	public WAE025MonthlyShippingListController() {
		// no process
	}

	/**
	 * [종합관리] - [판매정보] - 월출하지시현황  초기화면[KR] <br>
	 * 
	 * @param request HttpServletRequest
	 * @param form    WAE026MonthSalesPlanForm
	 * @return ModelAndView
	 * @throws Exception Exception
	 */
	@RequestMapping(value = "/init", method = { RequestMethod.GET, RequestMethod.POST })
	public ModelAndView initMonthlyShippingList(HttpServletRequest request,
		...
		mv.addObject("url", "WAE025MonthlyShippingList.jsp");
		mv.setViewName(".tiles.page.WAE025MonthlyShippingList");
		return mv;
	}

	/**
	 * [종합관리] - [판매정보] - 월출하지시현황 데이터 조회
	 * 
	 * @param request HttpServletRequest
	 * @param form    WAE026MonthSalesPlanForm
	 * @return ModelAndView
	...
	public ModelAndView monthlyShippingListSearch(HttpServletRequest request,
			@ModelAttribute("WAE025MonthlyShippingListForm") WAE025MonthlyShippingListForm form)
			throws Exception {
		...
		String year="";
		List<MonthlyShippingListDomain> monthShippingList = null;
		
		if(dwhDomain2 != null){
			year =dwhDomain2.getJyymm();
			jsonObj.put("year", year);
			try {
				monthShippingList = mifService.searchByMonthlyShippingList(form);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (monthShippingList != null) {
				result = DnkrConstans.RESULT_Y;
				jsonObj.put("list", monthShippingList);
			}
		}
