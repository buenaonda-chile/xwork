...
import com.globaldenso.dnkr.presentation.form.dwh.WAE009MonthlyDivisionRatioForm;
 ...
 */
@Controller
@RequestMapping(value = "/WAE009MonthlyDivisionRatioSearch")
@ViewId(value = "WAE009")
public class WAE009MonthlyDivisionRatioController {
	 ...
	 * [JP] デフォルトコンストラクタ。<br>
	 */
	public WAE009MonthlyDivisionRatioController() {
		// no process
	}
	...
	public ModelAndView initMonthlyDivisionRatioController(HttpServletRequest request,
			@ModelAttribute("WAE009MonthlyDivisionRatioForm")WAE009MonthlyDivisionRatioForm form)
			throws Exception {
		/**
		 * <pre>
		 *             
		...
		mv.addObject("url", "WAE009MonthlyDivisionRatio.jsp");
		mv.setViewName(".tiles.page.WAE009MonthlyDivisionRatio");
		return mv;
	}
	 ...
	 * 
	 * @param request HttpServletRequest
	 * @param form    WAE009MonthlyDivisionRatioForm
	 * @return ModelAndView
	 * @throws Exception
	 */
	@RequestMapping(value = "/ajaxSearch", method = { RequestMethod.GET, RequestMethod.POST })
	public ModelAndView monthlyDivisionRatioSearch(HttpServletRequest request,
			@ModelAttribute("WAE009MonthlyDivisionRatioForm") WAE009MonthlyDivisionRatioForm form)
			throws Exception {
		...
		String year="";
		List<MonthlyDivisionRatioDomain> oceansList = null;
				
		if(dwhDomain2 != null){
			year =dwhDomain2.getJyymm();
			jsonObj.put("year", year);
			try {
				oceansList = mifService.searchByMonthlyDivisionRatio(form);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (oceansList != null) {
				result = DnkrConstans.RESULT_Y;
				jsonObj.put("list", oceansList);
			}
		}
