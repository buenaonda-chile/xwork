...
import com.globaldenso.dnkr.presentation.form.dwh.WAE033ProductionRecordForm;
 ...
 */
@Controller
@RequestMapping(value = "/WAE033ProductionRecordSearch")
@ViewId(value = "WAE033")
public class WAE033ProductionRecordController {
	 ...
	 * [JP] デフォルトコンストラクタ。<br>
	 */
	public WAE033ProductionRecordController() {
		// no process
	}
	...
	public ModelAndView initProductionRecord(HttpServletRequest request,
		...
		mv.addObject("url", "WAE033ProductionRecord.jsp");
		mv.setViewName(".tiles.page.WAE033ProductionRecord");
		return mv;
	}
	...
	public ModelAndView productionRecordSearch(HttpServletRequest request,
			@ModelAttribute("WAE033ProductionRecordForm") WAE033ProductionRecordForm form)
			throws Exception {
		...
		String year="";
		List<ProductionRecordDomain> productionRecordList = null;
		
		if(dwhDomain2 != null){
			year =dwhDomain2.getJyymm();
			jsonObj.put("year", year);
			try {
				productionRecordList = mifService.searchByProductionRecord(form);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (productionRecordList != null) {
				result = DnkrConstans.RESULT_Y;
				jsonObj.put("list", productionRecordList);
			}
		}
