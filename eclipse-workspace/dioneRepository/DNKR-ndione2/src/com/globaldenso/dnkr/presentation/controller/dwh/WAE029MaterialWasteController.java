...
import com.globaldenso.dnkr.presentation.form.dwh.WAE029MaterialWasteForm;
 ...
 */
@Controller
@RequestMapping(value = "/WAE029MaterialWasteSearch")
@ViewId(value = "WAE029")
public class WAE029MaterialWasteController {
	 ...
	 * [JP] デフォルトコンストラクタ。<br>
	 */
	public WAE029MaterialWasteController() {
		// no process
	}
	...
	public ModelAndView initMaterialWaste(HttpServletRequest request,
		...
		mv.addObject("url", "WAE029MaterialWaste.jsp");
		mv.setViewName(".tiles.page.WAE029MaterialWaste");
		return mv;
	}
	...
	public ModelAndView materialWasteSearch(HttpServletRequest request,
			@ModelAttribute("WAE029MaterialWasteForm") WAE029MaterialWasteForm form)
			throws Exception {
		...
		String year="";
		List<MaterialWasteDomain> materialWasteList = null;
		
		if(dwhDomain2 != null){
			year =dwhDomain2.getJyymm();
			jsonObj.put("year", year);
			try {
				materialWasteList = mifService.searchByMaterialWaste(form);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (materialWasteList != null) {
				result = DnkrConstans.RESULT_Y;
				jsonObj.put("list", materialWasteList);
			}
		}
