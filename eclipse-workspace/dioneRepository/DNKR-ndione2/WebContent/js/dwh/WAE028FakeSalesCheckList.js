...
function search() {
	...
	if (val_chk(pJobFileName, "작업파일")) {
		...
		var objList = getAjaxList("/WAE028FakeSalesCheckListSearch/ajaxSearch.do", params)
	}
	return false;
}
...
function createGrid(){
	unitGrid = new ax5.ui.grid({
        ...
        columns: [
            ...
            {key: "cusnm", label: "업체명", width:250},
            {key: "itcls", label: "ITCLS",Aidth:50},
            {key: "pspno", label: "품번", width:200},
            {key: "itdsc", label: "품명", width:250},
            {key: "salct", label: "단가 ", width:90, formatter: "money", align: "right"},
            {key: "reasn", label: "이유", width:50},
            {key: "bfqty", label: "이월수량", width:90, formatter: "money", align: "right"},
            {key: "bfamt", label: "이월금액", width:90, formatter: "money", align: "right"},
            {key: "slqty", label: "판매수량", width:90, formatter: "money", align: "right"},
            {key: "slamt", label: "판매금액", width:90, formatter: "money", align: "right"},
            {key: "stqty", label: "정산수량", width:90, formatter: "money", align: "right"},
            {key: "stamt", label: "정산금액", width:90, formatter: "money", align: "right"}
        ]
	});
...
//	=== grid =================================================
	...
	$('.exel').on('click', function() {
		unitGrid.exportExcel("가매출CheckList.xls");
	});
//	=== //grid =================================================
