...
<c:if test="${mode ne 'view' }">
		<div class="fl">
				<ul class="pj-smtab">
					<li <c:if test="${view eq 'normal' }">class="over first"</c:if>><a href="/WAG004SurveyTarget/init.do?srvCode=${paraEmp.srvCode }&view=normal"><span><spring:message code="label.WAG001.individualSelect"/></span></a></li>
					<li <c:if test="${view eq 'team' }">class="over first"</c:if>><a href="/WAG004SurveyTarget/teamList.do?srvCode=${paraEmp.srvCode }&view=team"><span><spring:message code="label.WAG001.teamSelect"/></span></a></li>
				</ul>
		</div>
</c:if>
<%-- <form:form action="${pageContext.request.contextPath}/WAB001EmpSearch/search.do" method="POST" name="EmpDomain" id="WAD001EmpSearch"> --%>
	<form:form action='${pageContext.request.contextPath}/WAG004SurveyTarget/search.do?tabId=${tabId}&menu=${menu}' method="POST"  modelAttribute="TargetDomainForm" name="TargetDomainForm">
		<div class="ssgap"></div>
		...
		<table id="empTable">
			...
			<thead>
				<tr>
					...
					<th>
							<spring:message code='label.useYn'/><!-- 사용 -->
					</th>
				</tr>
			</thead>
			<tbody>
								...
								<c:set value="${fn:length(empList)}" var="empListSize"/>
									<c:forEach var="empDomain" items="${empList}" varStatus="status">
										<tr>
											...
											<c:choose>
												<c:when test="${empDomain.chkYn > 0 && mode ne 'view'}">
													<c:set var="checkYn">disabled="disabled"</c:set>
												</c:when>
											</c:choose>
											<input type="checkbox" ${checkYn } name="empChk" id="empList_${empDomain.teamCode }" value="${empDomain.empNumber }§${empDomain.empEmail}">
		...
		<div class="gap20"></div>
			...
			<c:if test="${fn:length(empList) > 0}">
				<div class="pagenation">
		            <smpl:seqpaging maxSeq="10" action='${pageContext.request.contextPath}/WAG004SurveyTarget/init.do?tabId=${tabId}&menu=${menu}' formId="TargetDomainForm" />
		        </div>
