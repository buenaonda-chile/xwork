...
<c:if test="${fn:substring(authBinaryNum, 3, 4) eq '1' }"><%--조회권한 --%>
	<form:form action="${pageContext.request.contextPath}/WAE001ManagementSearchController/MaterialCost/search.do?tabId=${tabId}&menu=${menu}" method="POST"  modelAttribute="WAE001MngSearchForm" name="WAE001MngSearchForm">
	<div class="board-bList-wrap log_board">
		...
		<table style="overflow: auto">
			...
			<tbody>
				<c:choose>
					<c:when test="${fn:length(materialCostList) > 0}">
						<c:forEach var="DataWareHouseDomain" items="${materialCostList}" varStatus="status">
						<tr>
		...
		<div class="gap20"></div>
			<c:if test="${fn:length(materialCostList) > 0}">
				<div class="paging-wrap">
		            <smpl:seqpaging maxSeq="10" action="${pageContext.request.contextPath}/WAE001ManagementSearchController/MaterialCost/search.do?tabId=${tabId}&menu=${menu}" formId="WAE001MngSearchForm" />
		        </div>
