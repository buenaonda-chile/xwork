...
public class PscstlibController extends MultiActionController {
	...
	***********************************************************************/ 		
	 ...
	 * 	- 2020-05-11	:	jhb
	***********************************************************************/ 		
	public void selectCJJE200Q_exchange(HttpServletRequest request, HttpServletResponse response) throws Exception {	
		PlatformRequest pReq 	= new PlatformRequest(request, "UTF-8"); 
		pReq.receiveData();
		Map pData = DataConverter.getMiPlatformData(pReq);
		List list = dao.getListData("pscstlib.selectCJJE200Q_exchange", pData);		
		ModelMap map = new ModelMap();
		map.addAttribute("ds_EXCHANGE", list);
		new MiPlatformView().render(map, request, response);
	}
	
	public void selectCJJE200Q(HttpServletRequest request, HttpServletResponse response) throws Exception {	
		PlatformRequest pReq 	= new PlatformRequest(request, "UTF-8"); 
		pReq.receiveData();
		Map pData = DataConverter.getMiPlatformData(pReq);
		List list = dao.getListData("pscstlib.selectCJJE200Q", pData);		
		ModelMap map = new ModelMap();
		map.addAttribute("ds_CJJE200Q", list);
		new MiPlatformView().render(map, request, response);
	}
