 ...
 */
@Controller
@RequestMapping(value = "/WAE019MaterialInventorySearch")
@ViewId(value = "WAE019")
public class WAE019MaterialInventoryController {
	 ...
	 * [JP] デフォルトコンストラクタ。<br>
	 */
	public WAE019MaterialInventoryController() {
		// no process
	}
	...
	public ModelAndView initMaterialInventory(HttpServletRequest request,
		...
		mv.addObject("url", "WAE019MaterialInventory.jsp");
		mv.setViewName(".tiles.page.WAE019MaterialInventory");
		return mv;
	}
	...
	public ModelAndView materialInventorySearch(HttpServletRequest request,
		...
		String year="";
		List<MaterialInventoryDomain> materialInventoryList = null;
		
		if(dwhDomain2 != null){
			year =dwhDomain2.getJyymm();
			jsonObj.put("year", year);
			try {
				materialInventoryList = mifService.searchByMaterialInventory(form);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (materialInventoryList != null) {
				result = DnkrConstans.RESULT_Y;
				jsonObj.put("list", materialInventoryList);
			}
		}
