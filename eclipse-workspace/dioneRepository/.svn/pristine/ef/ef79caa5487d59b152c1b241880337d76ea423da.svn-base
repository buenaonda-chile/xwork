...
function search() {
	...
	if (val_chk(pJobFileName, "작업파일")) {
		...
		var objList = getAjaxList("/WAE024BusinessShippingSearch/ajaxSearch.do", params)
	}
	return false;
}
...
function createGrid(){
	unitGrid = new ax5.ui.grid({
        ...
        columns: [
            ...
            {key: "pyvnd", label: "거개처코드"},
            {key: "cusnm", label: "거래처명", width:200},
            {key: "pcost", label: "단가", formatter: "money", align: "right"},
            {key: "jpsqy", label: "출하수량", formatter: "money", align: "right"},
            {key: "jpsam", label: "금액", width:150, formatter: "money", align: "right"}
        ]
	});
}
...
//	=== grid =================================================
	...
	$('.exel').on('click', function() {
		unitGrid.exportExcel("영업출하현황.xls");
	});
//	=== //grid =================================================
