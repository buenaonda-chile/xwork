...
function validation(item){
	...
	if(item.prdtCode==null || wijmo.isEmpty(item.prdtCode) || item.prdtCode ==""){
		toastMsg("수정이나 추가한 행의 [제품코드]컬럼을 입력 하세요");
		return false;
	}else{
		var flag = isCodeValid(item.prdtCode);
		if(!flag) {
	...
	if(item.prdtNm==null || wijmo.isEmpty(item.prdtNm) || item.prdtNm ==""){
		toastMsg("수정이나 추가한 행의 [제품명]컬럼을 입력 하세요");
		return false;
	}
...
function search(){
	...
	var url = "/ajaxProdctCodeBase/search.do";	
	var params = {
		'code' : $("#code").val(),
		'name' : $("#name").val()		
	};
...
//	} 
    ...
    for ( var i = 0; i <cv.itemsEdited.length; i ++) {
		...
		$.ajax({
			method : "post",
			url : "/ajaxProdctCodeBase/update.do",
    ...
    for ( var i = 0; i <cv.itemsAdded.length; i ++) {
		...
		$.ajax({
			method : "post",
			url : "/ajaxProdctCodeBase/create.do",
    ...
    for ( var i = 0; i <cv.itemsRemoved.length; i ++) {
		...
		$.ajax({
			method : "post",
			url : "/ajaxProdctCodeBase/delete.do",
...
function downloadExcel(){
	...
	var fileName = "제품코드마스타_"+getToday()+".xlsx";
...
//	var comsBase = new DataMap(coms_base, 'id', 'name');
	var columnsDefinition=[			
		{binding:'prdtCode',header: '* '+ LB_PRDT_CODE,width: 200, align:'center',dataType:'String', maxLength:1},
		{binding:'prdtNm',header: '* '+ LB_PRDT_NM, width: '*', align:'left',dataType:'String', maxLength:50},
		{binding:'startDe',header: '* '+ LB_START_DE, width: 200, align:'center',dataType:'String', maxLength:10},
		{binding:'endDe',header: '* '+ LB_END_DE, width: 200, align:'center',dataType:'String', maxLength:10},		
		{binding:'refrn',header: '* '+ LB_REFRN, width: '*', align:'left',dataType:'String', maxLength:50},
	    {binding:'oldPrdtCode',visible:false},
	    {binding:'oldStartDe',visible:false},
	    {binding:'oldEndDe',visible:false},
