...
System.out.println(emptyPN);
	...
	***********************************************************************/
	 ...
	 * 	- HttpServletResponse response		:	데이터셋
	 * @exception 
	 * 	-  [설비보전]-전라인가동현황-라인코드 맵핑 마스터 (TEST용)
	 * @see
	 * 	- 2019-10-31	:	제환복
	***********************************************************************/
	public void selectOBMZ230Q_IF(HttpServletRequest request, HttpServletResponse response) throws Exception {	
		...
		while (chk.hasNext()) {
			...
			*/
			...
			pData.put("PLINCD", chkData.get("MACH_CODE").toString().trim());	//보전진척 라인코드
			pData.put("PLINNM", chkData.get("MACH_DESC").toString().trim());	//보전진척 라인명
			pData.put("CURITEM", chkData.get("CUR_ITEM_CODE").toString().trim());	//현재 생산품목
			pData.put("CURRATE", chkData.get("CUR_ING_RATE").toString().trim());	//생산진척 설총률
			pData.put("MODTM", chkData.get("CUR_ING_DATETIME").toString().trim());	//생산진척 갱신시간
	 ...
	 * 	- HttpServletResponse response		:	데이터셋
	 * @exception 
	 * 	-  [설비보전]-전라인가동현황-라인/설비 장애상태관리
	 * @see
	 * 	- 2020-07-22	:	제환복
	***********************************************************************/
	public void selectOBMZ270(HttpServletRequest request, HttpServletResponse response) throws Exception {	
		PlatformRequest pReq 	= new PlatformRequest(request, "UTF-8"); 
		pReq.receiveData();
		Map pData = DataConverter.getMiPlatformData(pReq);
		List list = dao.getListData("psogmlib.selectOBMZ270", pData);
		ModelMap map = new ModelMap();
		map.addAttribute("ds_OBMZ270", list);
		new MiPlatformView().render(map, request, response);
	}
	public void saveOBMZ270(HttpServletRequest request, HttpServletResponse response) throws Exception {	
		PlatformRequest pReq = new PlatformRequest(request);
		pReq.receiveData();
		List pList = DataConverter.getMiPlatformMultiData(pReq, "ds_OBMZ270");
		Map pArg = DataConverter.getMiPlatformData(pReq);
		...
		DatasetList in_dl = new DatasetList();
		in_dl = pReq.getDatasetList();
		Dataset ds = in_dl.getDataset("ds_OBMZ270");
		//////////////////////////////////////////////////////////////		
		...
		while (it_iu.hasNext()) {
			Map pData = (Map) it_iu.next();
			pData.put("LIB2", (String)pArg.get("LIB2"));
			emptyPN = (String) pData.get("STATU_CHG");
			rowType = (String) pData.get("ROW_TYPE");
			if (("1").equals(emptyPN)){
				if ("update".equals(rowType)) {
					Map CheckCnt = (Map)dao.getReadData("psogmlib.selectOBMZ270_Check",pData);
					if(CheckCnt!=null && CheckCnt.get("CNT")!=null && !CheckCnt.get("CNT").equals("")){
						if(CheckCnt.get("CNT").equals(1)){
							System.out.println("status1");
							dao.updateData("psogmlib.updateOBMZ270_MSHNO", pData);
							dao.updateData("psogmlib.updateOBMZ270_LINCD", pData);
						}else{
		...
		dao.endTransaction(); //commit
		ModelMap map = new ModelMap();//추가
		map.addAttribute("ds_OBMZ270_CHECK", errList);//추가
		new MiPlatformView().render(map, request, response);
	}
}
