...
<c:url var="readUrl" value="/subCategory/search"/>
<c:url var="createUrl" value="/subCategory/create"/>
<c:url var="updateUrl" value="/subCategory/update"/>
<c:url var="deleteUrl" value="/subCategory/delete"/>
...
<div class="sub-con-wrap">
  <h2 class="subpage-tit">현상관리 소</h2>
  <!-- 버튼들 -->
    ...
    <fieldset>
      <table>
        ...
        <tr>
          <th>분류검색</th>
          <td>
            <input type="text" id="txt_mainCategory" value="" title="분류검색"
                   data-bind="value: criteria.mainCategory"/>
          </td>
          <td class="explain">대·소분류명,코드</td>
          <th>사용여부</th>
          <td>
      ...
      <form id="frm_input" method="post">
        ...
        <fieldset>
          <table class="bWrite">
            ...
            <tbody>
            <tr>
              <th scope="row"><label for="cmb_mainCategory_input">대분류명</label>
              </th>
              <td>
                <kendo:dropDownList name="cmb_mainCategory_input" dataTextField="mainCategoryNm"
                                    dataValueField="mainCategory"
                                    data-value-primitive="true"
                                    data-bind="value: data.mainCategory">
                  <kendo:dataSource data="${mainCategoryItems2}"/>
                </kendo:dropDownList>
              </td>
            </tr>
		
            <tr>
              <th scope="row"><label for="smallCategory_input">소분류코드</label>
              </th>
              <td>
                <input type="text" id="smallCategory_input" data-bind="value: data.smallCategory" />
              </td>
            </tr>
		
            <tr>
              <th scope="row"><label for="smallCategoryNm_input">소분류명</label>
              </th>
              <td>
                <input type="text" id="smallCategoryNm_input" data-bind="value: data.smallCategoryNm" />
              </td>
            </tr>
		            <tr>
              <th scope="row">
...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    ...
    cmbmainCategoryInput = $("#cmb_mainCategory_input", "#frm_input").data("kendoDropDownList");
    var mainCategoryData = new kendo.data.DataSource({ data: ${mainCategoryItems}}), mainCategoryItems = getEditOptions2(mainCategoryData.options.data, "mainCategory");
	...
	,{ title: '대분류명', name: 'mainCategory', align: 'center', editOptions: { type: 'select', listItems: mainCategoryItems, useViewMode: true } }
	,{ title: '소분류코드', name: 'smallCategory', align: 'center' }
	,{ title: '소분류명', name: 'smallCategoryNm', align: 'center' }
	,{
          title: '사용여부',
          name: 'useYn',
          align: 'center',
	...
	,{ title: '부품구분 ', name: 'partDiv', align: 'center', editOptions: { type: 'select', listItems: partDivItems, useViewMode: true } }
