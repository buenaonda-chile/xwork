...
//2020.03.12 Win10 excel DATA소실관련- 제환복추가
function createExcelSheetS4(fdObj,fName,arrSheets, postGrid)
{
	...
	var fileFullName= fdObj.FilePath + fName;	//파일경로+파일명
	var rtnTemp = postGrid.SaveExcel(fileFullName + "Temp.xls" ,sheets[0],"A1",false,true,false,false); //임시저장

	if(!rtnTemp) {
		trace("임시파일저장완료");
		var rtn = ext_ExcelSaveAsEx(fileFullName + "Temp.xls" , fileFullName + ".xlsx", "2007", false);
		
		//임시파일 삭제
		File.Delete(fileFullName + "Temp.xls");
