...
<c:url var="createUrl" value="/models/create"/>
<c:url var="updateUrl" value="/models/update"/>
...
<div class="sub-con-wrap">
  <h2 class="subpage-tit">차종관리</h2>
  <!-- 버튼들 -->
  <div class="sub-btn-group">
    <button id="btn_search" class="button" data-bind="click: search, visible: btnGrp0Visible"><i
            class="w_search"></i>검색
    </button>
    <button id="btn_add" class="button bc1" data-bind="click: add, visible: btnGrp0Visible"><i
            class="w_add"></i>추가
    </button>
    <button id="btn_save" class="button bc2" data-bind="click: save, visible: btnGrp1Visible"><i
            class="w_save"></i>저장
    </button>
    <button id="btn_cancel" class="button" data-bind="click: cancel, visible: btnGrp1Visible"><i
            class="w_cancel"></i>취소
    </button>
    <button id="btn_remove" class="button" data-bind="click: remove, visible: btnGrp2Visible"><i
            class="w_minus"></i>삭제
    </button>
  </div>
  <!-- /버튼들 -->
      ...
      <form id="frm_input" method="post">
        ...
        <fieldset>
          <table class="bWrite">
            ...
            <tbody>
            <tr>
              <th scope="row"><label for="cmb_tireStd_input">승상용</label>
              </th>
              <td>
                <kendo:dropDownList name="cmb_tireStd_input" dataTextField="codeNm"
                                    dataValueField="codeCd"
                                    data-value-primitive="true"
                                    data-bind="value: data.tireStd">
            ...
            <tr>
              <th scope="row"><label for="model_input">차종</label>
              </th>
              <td>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        cmbTireStdInput = $("#cmb_tireStd_input", "#frm_input").data("kendoDropDownList");
        var tireStdData = cmbTireStdInput.dataSource.data(),
            tireStdItems = _dicasPage.getEditOptions(tireStdData);
        ...
        var makerData = cmbMakerInput.dataSource.data(),
            makerItems = _dicasPage.getEditOptions(makerData);

        exhaustRegInput = $("#cmb_exhaustReg_input", "#frm_input").data("kendoDropDownList");
        var exhaustRegData = exhaustRegInput.dataSource.data(),
            exhaustRegItems = _dicasPage.getEditOptions(exhaustRegData);
        ...
        var engineTypeData = cmbEngineTypeInput.dataSource.data(),
            engineTypeItems = _dicasPage.getEditOptions(engineTypeData);
        ...
        grid = new tui.Grid({
            ...
            columns: [
                ...
                // ,{ title: '이관여부', name: 'tranYn', align: 'center' }
            ]
        });
