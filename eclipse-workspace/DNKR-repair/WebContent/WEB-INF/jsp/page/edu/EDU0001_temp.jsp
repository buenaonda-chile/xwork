...
<script type="text/javascript">
    ...
    var buttonView = kendo.observable({
        ...
        add: function () {
          ...
          grid.setValue(gridRowkey, "ID", grid.getValue(inputView.get('rowKey'), "ID", true));
        ...
        save: function() {
			...
			if (grid.getModifiedRows().createdRows.length > 0) {
				...
				for (var i = 0; i < grid.getModifiedRows().createdRows.length; i++){			
					if(grid.getModifiedRows().createdRows[i].eduDiv == ""){
						alert("교육대상을 선택 해주세요.");
						return;
					}
					
					if(grid.getModifiedRows().createdRows[i].eduNm == ""){
						alert("교육명을 입력 해주세요.");
						return;
					}
					
					if(grid.getModifiedRows().createdRows[i].eduDt == ""){
						alert("교육일시를 입력 해주세요.");
						return;
					}
					
					if(grid.getModifiedRows().createdRows[i].receiptEndDate == ""){
						alert("접수마감일시를 입력 해주세요.");
						return;
					}
					
					if(grid.getModifiedRows().createdRows[i].persCnt <= 0){
						alert("인원수를 입력 해주세요.");
						return;
					}
					
					if(grid.getModifiedRows().createdRows[i].onoffYn == ""){
						alert("온/오프라인을 선택 해주세요.");
						return;
					}
				}
				...
				for (var i = 0; i < grid.getModifiedRows().createdRows.length; i++){
					...
					_dicasPage.maskOpen();
					/* alert(JSON.stringify(grid.getModifiedRows().createdRows[i])); */
					 $.ajax({
				          method: "POST",
				          url: '${createUrl}',
			...
			if (grid.getModifiedRows().updatedRows.length > 0) {
				...
				for (var i = 0; i < grid.getModifiedRows().updatedRows.length; i++){			
					if(grid.getModifiedRows().updatedRows[i].eduDiv == ""){
						alert("교육대상을 선택 해주세요.");
						return;
					}
					
					if(grid.getModifiedRows().updatedRows[i].eduNm == ""){
						alert("교육명을 입력 해주세요.");
						return;
					}
					
					if(grid.getModifiedRows().updatedRows[i].eduDt == ""){
						alert("교육일시를 입력 해주세요.");
						return;
					}
					
					if(grid.getModifiedRows().updatedRows[i].receiptEndDate == ""){
						alert("접수마감일시를 입력 해주세요.");
						return;
					}
					
					if(grid.getModifiedRows().updatedRows[i].persCnt <= 0){
						alert("인원수를 입력 해주세요.");
						return;
					}
					
					if(grid.getModifiedRows().updatedRows[i].onoffYn == ""){
						alert("온/오프라인을 선택 해주세요.");
						return;
					}
				}
    ...
    $(document).ready(function () {
	    ...
	    grid = new tui.Grid({
	        ...
	        columns: [
	        	...
	        	,{ title: 'CHK', name: 'chk', align: 'center', hidden: 'true' }
	            ,{ title: '교육대상', name: 'eduDiv', align: 'center'
	            	, editOptions: {type: 'select', listItems: educationItems, useViewMode: false}
	             }
	            ,{ title: '교육명', name: 'eduNm', align: 'center'
	            	, editOptions: { type: 'text', maxLength: 20, useViewMode: false }
	             	, onAfterChange: function(ev) {
					  grid.setValue(ev.rowKey, "chk", 'U');
	              	}
	             }
	            ,{ title: '교육일시', name: 'eduDt', align: 'center'
	            	, editOptions: { type: 'text', maxLength: 12, useViewMode: false}
	             	, onAfterChange: function(ev) {
	             		grid.setValue(ev.rowKey, "chk", 'U');
	              	} 
	             }  

	            ,{ title: '접수마감일시', name: 'receiptEndDate', align: 'center'
	            	, editOptions: { type: 'text', maxLength: 12, useViewMode: false}
	             	, onAfterChange: function(ev) {
	            		grid.setValue(ev.rowKey, "chk", 'U');
	              	}
	             }
	            ,{ title: '인원수', name: 'persCnt', align: 'center'
	            	, editOptions: { type: 'text', maxLength: 3, useViewMode: false }
	             	, onAfterChange: function(ev) {
	         			grid.setValue(ev.rowKey, "chk", 'U');
	             	}
	             }
	            ,{ title: '온/오프라인', name: 'onoffYn', align: 'center' 
	            	 , editOptions: {type: 'select', listItems: onoffItems, useViewMode: false}         
	             }
	            ,{ title: '사용유무', name: 'useYn', align: 'center',
	                    ...
	                    if (ev.value == 'Y') {
	                      ...
	                      for (var i=0; i<cnt; i++) {
	                        ...
	                        if (gridData.rowKey != ev.rowKey) {
	                        	grid.setValue(gridData.rowKey, 'useYn', null);
	                        }
	                      }
	                    }
	                  }
	            }
	            ,{ title: '비고', name: 'remark', align: 'center'
	            	, editOptions: { type: 'text', maxLength: 50, useViewMode: false }
	             	, onAfterChange: function(ev) {
					  grid.setValue(ev.rowKey, "chk", 'U');
	              	} 
	             }
