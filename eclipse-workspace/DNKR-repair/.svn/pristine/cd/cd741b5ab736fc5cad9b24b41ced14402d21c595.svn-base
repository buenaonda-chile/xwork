...
var sdat = new wijmo.input.InputDate("#searchSalDtFr");	//판매일자 시작
		
var edat = new wijmo.input.InputDate("#searchSalDtTo");	//판매일자 종료

var dat = new wijmo.input.InputDate(document.createElement('div')); //판매일자
...
var handleEvent = function(){
	...
	$("#btn_add").on("click", function() {
		...
		var date = new Date();
		addRow.saleDt = getFormatDate(date);
		addRow.saleNo = "[판매번호는 자동생성됩니다.]";
...
var validation = function(item){
	...
	if (item.saleDt == null || wijmo.isEmpty(item.saleDt) || item.saleDt == "") {
		alert("[판매일자를 입력 해주세요.]");
		return false;
	}
...
var save = function(){
	...
	for ( var i = 0; i <cv.itemsAdded.length; i ++) {
		...
		ajaxCommit("/saleSDRepairItem/create",cv.itemsAdded[i]);
	...
	for ( var i = 0; i <cv.itemsEdited.length; i ++) {
		...
		ajaxCommit("/saleSDRepairItem/update",cv.itemsEdited[i]);
	}
	...
	for ( var i = 0; i <cv.itemsRemoved.length; i ++) {
		...
		ajaxCommit("/saleSDRepairItem/delete",cv.itemsRemoved[i]);
...
var search = function(){
	...
	var url = '/saleSDRepairItem/search';
	
	var params = {
		"searchSalDtFr" : wijmo.Globalize.format(sdat.value,'yyyy-MM-dd'),
		"searchSalDtTo" : wijmo.Globalize.format(edat.value,'yyyy-MM-dd'),
		"saleNo" : $("#rectNo").val(),
		"partNo" : $("#partNo").val(),
		"custNm" : $("#custNm").val(),
		"page":1
...
var gridInit = function() {
	...
	columnsDefinition = [
	    ...
	    {binding: 'saleNo', header: '판매번호', width: '*', align:'center',dataType:'Number',isReadOnly:true},  
        {binding: 'saleDt', header: '판매일자', width: '*', align:'center',dataType:'String',maxLength:12}, //wordWrap: true: 텍스트 자동줄바꿈 autorowHeights랑 같이 사용
		...
		{binding: 'qty', header: '수량', width: '*', align:'center',dataType:'String'},								  
  		{binding: 'custNm', header: '판매처', width: '*', align:'center',dataType:'String',isReadOnly:true},
  		{binding: 'custId', header: '판매처아이디', width: '*', align:'center',dataType:'Number',visible:false,isReadOnly:true},
		{header: '', width: 40, align:'center',
					cellTemplate: cellMaker.makeButton({
								text:"<img src='/assets/img/lib/ic_search1.png'>",
								click: function(e, ctx) {
        ...
        {binding: 'custEmpNm', header: '판매처담당자', width: '*', align:'center',dataType:'String',isReadOnly:true},
