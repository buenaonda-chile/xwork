...
var handleEvent = function(){
	...
	$("#btn_remove").on("click", function() {
		if(cv.currentItem.status == 'R4' || cv.currentItem.status == 'R2'){
			alert("[반납된 품목은 삭제불가능합니다.]");
		}else{
...
var getReturnValue = function (returnValue,value) {
	...
	if(value=='part'){
			...
			flexGrid.setCellData(custNm, 'custNm', retObj.custNm, true);
			flexGrid.setCellData(custNm, 'custId', retObj.custId, true);
	}
}
...
var save = function(){
	...
	for ( var i = 0; i <cv.itemsAdded.length; i ++) {
		...
		ajaxCommit("/receiptSDBrkPart/create",cv.itemsAdded[i]);
	...
	for ( var i = 0; i <cv.itemsEdited.length; i ++) {
		...
		ajaxCommit("/receiptSDBrkPart/update",cv.itemsEdited[i]);
	}
	...
	for ( var i = 0; i <cv.itemsRemoved.length; i ++) {
		...
		ajaxCommit("/receiptSDBrkPart/delete",cv.itemsRemoved[i]);
...
var search = function(){
	...
	var url = '/receiptSDBrkPart/search';
