...
var LN_INS_CHK5 = "<spring:message code='label.WAT000.insChk5'/>";
var LN_INS_CHK6 = "<spring:message code='label.WAT000.insChk6'/>";
	...
	<div class="gridContents" style="" id="grid-parent">
		<div style="margin: 0px 15px 0px 0px;">
	  			...
	  			<div class="k_sg4">
					<div class="board-bList-wrap log_board">
						<div class="gridBtnBlock k_sg1">
							...
							<button type="button" class="AXButton Normal" onclick="paymentRequest();">
								<spring:message code='label.WAN010.PaymentRequest'/><!-- 결제요청 -->
							</button>								
						</div>	
					</div>
				</div>
			...
			<div>
	  			<div class="k_sg4">
					<div class="board-bList-wrap log_board">
						<label class="k_l1">지원받은시간(분)</label>	
						<input type="text" id="getRcvsupTm" style="font-size: 15px;text-align: center;" onkeydown='return onlyNumber(event)'  onkeyup='removeChar(event)' >	
						<button type="button" class="AXButton Normal" style="" onclick="return rcvsupTmUpdate()">
							<spring:message code="label.save" /><!-- 저장 -->
						</button>						
						<label class="k_l1" style="margin-left: 20px;">물류제외 총작업시간(분)</label>	
						<input type="text" id="getTtlWork" style="font-size: 15px;text-align: center;background-color:#C0C0C0" readOnly>						
					</div>
				</div>
			</div>
...
<script>
	$.getScript('/js/prdind/StEmpAbsCnf.js');
</script>
