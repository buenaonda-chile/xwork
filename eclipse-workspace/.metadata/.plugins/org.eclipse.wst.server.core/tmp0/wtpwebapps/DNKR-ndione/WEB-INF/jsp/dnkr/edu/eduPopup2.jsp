...
$(document).ready(function(){
	...
	$('#jqGridList3').jqGrid({ 
		...
		gridComplete : function() {
			var rows = $( "#jqGridList3" ).getDataIDs();
			for ( var i = 0; i < rows.length; i++ ) {
				$( "#jqGridList3" ).jqGrid( 'setCell', rows[i], 'loc_code', '', { cursor : 'pointer' } );
				$( "#jqGridList3" ).jqGrid( 'setCell', rows[i], 'rel_name', '', { cursor : 'pointer' } );
			}
		},
		onCellSelect: function(rowid, iCol, e){ // cell을 클릭 시

         	var item = $("#jqGridList3").jqGrid('getRowData', rowid);

			$("#team_code").val(item.team_code);
			$("#loc_code").val(item.loc_code);
			fn_emp_search();
	...
	$('#jqGridList4').jqGrid({ 
	    ...
	    //ajaxGridOptions: { contentType: "application/json;charset=utf-8" },
	    postData : fn_getFormData('#application_form'),
	    colNames : ['사원 코드', '사원명', 'OPER'],
	    colModel : [
					{name:'emp_number',index:'emp_number', width:100, align:'center'},
					{name:'emp_name',index:'emp_name', width:120, align:'center'},
					{name:'oper', width:50, align:'center', sortable:true, title:false, hidden: true}
				],
	    gridview: true,
	    toolbar: [false, "bottom"],
	    ...
	    onPaging: function(pgButton) {
    		...
    		var pageIndex         = parseInt($(".ui-pg-input").val());
   			var currentPageIndex  = parseInt($('#jqGridList4').getGridParam("page"));// 페이지 인덱스
   			var lastPageX         = parseInt($('#jqGridList4').getGridParam("lastpage"));  
   			var pages = 1;
   			var rowNum 			  = 100;	   
	...
	fn_insideGridresize( $(window), $( "#left4" ), $( "#jqGridList4" ), -150, 0.5 );
	...
	$("#btnSave").click(function(){
		...
		//아이템 체크 Validation
		var selarrrow = $("#jqGridList4").jqGrid('getGridParam', 'selarrrow');
		...
		var str = opener.$("#p_emp_cd").val();
		if(str.split(",").length > 0) {
			for(var i=0; i<str.split(",").length; i++) {
				if(str.split(",")[i] != '') {
					emp_pk.push(str.split(",")[i]);
				}
			}			
		}
		
		var strNm = opener.$("#p_emp_name").val();
		if(strNm.split(",").length > 0) {
			for(var i=0; i<strNm.split(",").length; i++) {
				if(strNm.split(",")[i] != '') {
					emp_name.push(strNm.split(",")[i]);
				}
			}			
		}
		
		for(var i=0; i<selarrrow.length; i++) {
			var item = $("#jqGridList4").jqGrid( 'getRowData', selarrrow[i]);
			emp_pk.push(item.emp_number);
			//emp_name.push($("#emp_name").val() + "_" + item.emp_name);
			emp_name.push(item.emp_name);
		}
		
		var resultArr = [];
		var resultNameArr = [];
		resultArr = removeDuplicatesArray(emp_pk);
		resultNameArr = removeDuplicatesArray(emp_name);
		
		opener.$("#p_emp_cd").val(resultArr);
		opener.$("#p_emp_name").val(resultNameArr);
		opener.$("#p_emp").text(resultNameArr);
...
function fn_emp_search() {
	
	//검색 시 스크롤 깨짐현상 해결
	$( "#jqGridList4" ).closest(".ui-jqgrid-bdiv").scrollLeft(0); 
	
	var sUrl = "/getEmpCodeList.do";
	$( "#jqGridList4" ).jqGrid( "clearGridData" );
	$( "#jqGridList4" ).jqGrid( 'setGridParam', {
		url : sUrl,
		mtype : 'POST',
		datatype : 'json',
		page : 1,
...
<input type="hidden" id="team_code" name="team_code" value=""/>
        ...
        <div class="layout-LW">
            ...
            <div style="width: 1000px;">
                ...
                <div class="left" style="width: 300px;" id="left1">
                    ...
                    <col />
                    <h2 class="subpage-tit">개인별권한설정</h2>
                ...
                <div class="left" style="width: 300px; margin-left: 15px;" id="left3">
                    ...
                    <h2 style="background-image:none;">&nbsp;</h2>
                    <div class="enrollment_center_3">
                        <input type="text" id="empQuery" name="empQuery" placeholder="Search..." value="" />
                        <a class="button" id="btnSearch4"><i class="w_cancel"></i>검색</a>
                    </div>
                    <br>
                    <table id="jqGridList4" class="bWrite" ></table>
                </div>
