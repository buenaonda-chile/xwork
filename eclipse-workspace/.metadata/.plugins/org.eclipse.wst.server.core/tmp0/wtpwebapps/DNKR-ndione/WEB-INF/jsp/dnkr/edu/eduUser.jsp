		...
		success: function (data) {
			...
			if ( data.result == 'fail' ) {
                ...
                for (var i = 0; i < data.list.length; i++) {
                    ...
                    $('.pk').val(item.pk);
                    fn_printFile(item);
                    if(item != "" && ( item.org_file_name.indexOf(".mp4") > 0 || item.org_file_name.indexOf(".ogg") > 0 || item.org_file_name.indexOf(".ogv") > 0 || item.org_file_name.indexOf(".webm") > 0) ) {
                        fn_printVideo(item);
                        videocontrol = document.getElementById('videoplay');
...
function fn_printFile(data){
    ...
    if(data !="" &&( data.org_file_name.indexOf(".mp4") > 0 || data.org_file_name.indexOf(".ogg") > 0 || data.org_file_name.indexOf(".ogv") > 0 || data.org_file_name.indexOf(".webm") > 0)){}
    else $("#file_list").append(str);
}

// 교육동영상 표시
function fn_printVideo(data){
	
    var str2 ="";
    str2 = "<table class=\"bWrite\"><tbody><div class=\"edu_video\" >" 
    	+"<h2 class='subpage-tit'><spring:message code='label.eduUser12'/></h2><input type='button' value='재생/일시정지' onclick='play();' style='border-radius:10px; float: right; border:1px solid; background-color:#D93232; font-size: 13px; color:white; margin : 10px 10px 0px 0px; padding:5px;'><div id='playtime' style = 'float: center;'></div>"
        + "<td style=\"text-align: left;\"> <ul id=\"video_file\"></ul></td>"
    	+ "</div></tbody></table>"
    $("#edu_video").append(str2);
	
    var str = "";
    str = "<li class='video_list' id='li_" + data.pk + "' >" 
    + "<video id='videoplay'src='http://ndione.densokorea.com:81/"+ data.file_name + "' width='100%' height='550px'>"
//    + "<video id='videoplay'src='http://10.73.254.199:81/"+ data.file_name + "' width='100%' height='550px'>"
//    + "<video id='videoplay'src='http://localhost/"+ data.file_name + "' width='100%' height='550px'>" 
//    + "</video></li><br> <div id='playtime'></div> <input type='button' value='재생/일시정지' onclick='play();' '> ";
    + "</video></li>";

    $("#video_file").append(str);
...
function play(){
    ...
    if(videocontrol.paused) {
    	if(videostatus == "N") {}
    	else if(videostatus == "videoMap_null") {
    		videostatus = "N";
    		videoStartTime(); // insert s_time
    	}
    	
    	
    	if(pauseCheck == true) var checkresult = videoCountCheck();
...
function PlayTime(e){
    ...
    "재생 상태 : " + Math.floor(videocontrol.currentTime) + "/" + Math.floor(videocontrol.duration);
}
...
function videoCountCheck(){
	...
	$.ajax({
		url: "eduVideoCount.do?edu_pk="+$("#edu_pk").val(),
		data: formData,
		dataType: "json",
		async: false,
		type: "POST",
...
function fn_save() {
	...
	if(videoyn == "Y" && videostatus != "Y") {	alert('교육 동영상 시청을 완료해 주세요.'); }
	else {
	...
	if($("#description_use").val() == 'Y') {
		...
		// 교육의견 10자 이내인 경우 에러 표시
		if($("#p_descript").val().length < 10) {
			alert("교육의견을 10자이상 입력해 주십시오.");
			return false;
		}
    ...
    <form id="application_form" name="application_form" enctype="multipart/form-data" method="post">
        ...
        <div class="doc-wrap" id="docBaseWrap">
            <div class="layout-LW" style="width:60%;">
                ...
                <div class="left_2" id="edu_video" style="width:100%;">
