...
$(document).ready(function(){
	...
	$('#jqGridList').jqGrid({ 
	    ...
	    colNames : ['SEQ', '연봉제목', '전체인원','미완료자수','완료자수','체결기간', '등록일', '등록자', '등록자ID','OPER'],
	    colModel : [
					{name:'pk',index:'pk', width:55, hidden : true},
					{name:'subject',index:'subject', width:200},
					//총인원
					{name:'edu_user_cnt',index:'edu_user_cnt', width:50, sortable:false, align:'center'},
					//미완료자수
					{name:'edu_user_notcompletion',index:'edu_user_notcompletion', width:50, sortable:false, align:'center'},
					//완료자수
					{name:'edu_user_completion',index:'edu_user_completion', width:50, sortable:false, align:'center'},
					{name:'start_date',index:'start_date', width:120, align:'center'},
					{name:'mod_date',index:'mod_date', align:'center', width:50},
					{name:'mod_user',index:'mod_user', width:50, sortable:false, align:'center'},

					{name:'reg_user_id',index:'reg_user_id', width:50, sortable:false, align:'center', hidden: false},
		...
		onCellSelect : function(row_id, iCol) {
			...
			if(row_id != null) {
				...
				if($("#emp_number").val() == item.reg_user_id) {
					location.href = "salaryRltDtl.do?edu_pk=" + item.pk;
				}
	        }
		}
	}); //end of jqGrid
