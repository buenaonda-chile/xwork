...
$(document).ready(function(){
	...
	$('#jqGridList').jqGrid({ 
	    ...
	    colNames : ['SEQ', '문제', '답안', '등록일', '등록자', 'OPER'],
	    colModel : [
					{name:'pk',index:'pk', width:55, hidden : true},
					{name:'subject',index:'subject', width:300 },
					{name:'ans_subject',index:'ans_subject', width:300 },
					{name:'mod_date',index:'mod_date', align:'center', width:80},
					{name:'mod_user',index:'mod_user', width:80, sortable:false, align:'center'},
		...
		gridComplete : function() {
			...
			for ( var i = 0; i < rows.length; i++ ) {
				...
				$( "#jqGridList" ).jqGrid( 'setCell', rows[i], 'ans_subject', '', { cursor : 'pointer' } );
		...
		onCellSelect : function(row_id, iCol) {
			...
			if(row_id != null) {
				...
				location.href = "queDetail.do?edu_pk=" + $("#edu_pk").val() + "&que_pk=" + item.pk;
	        }
		}
	}); //end of jqGrid
	...
	$("#btnReg").click(function(){
		location.href = "queDetail.do?edu_pk=" + $("#edu_pk").val();
	});
...
<input type="hidden" id="edu_pk" name="edu_pk" value="${edu_pk }" />
    ...
    <div class="layout-LW">
        ...
        <div class="left" style="width:923px;">
            ...
            <col />
            <h2 class="subpage-tit"><spring:message code='label.queList1'/></h2>
            <div class="search">
                <a class="button" id="btnSearch"><i class="w_search"></i> <spring:message code='label.queList2'/></a>
                <div class="center_search_container">
                    <span><spring:message code='label.queList3'/></span>
            ...
            <div class="enrollment">
                <a class="button" id="btnReg"><i class="w_add"></i><spring:message code='label.queList4'/></a>
                <a class="button" id="btnEdu"><i class="w_add"></i><spring:message code='label.queList5'/></a>
            </div>
        </div>
        <!-- 가운데 잡아줌// -->
