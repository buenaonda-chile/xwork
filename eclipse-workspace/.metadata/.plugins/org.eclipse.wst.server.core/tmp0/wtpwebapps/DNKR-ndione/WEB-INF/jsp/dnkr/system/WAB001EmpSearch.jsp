	...
	<form:form action='${pageContext.request.contextPath}/WAB001EmpSearch/search.do?tabId=${tabId}&menu=${menu}' method="POST"  modelAttribute="WAB001EmpSearchForm" name="WAB001EmpSearchForm">
		<div class="list-top text-right">
				...
				<c:if test="${fn:substring(authBinaryNum, 2, 3) eq '1' }"><%--등록권한 --%>
					<button type="button" class="AXButton Red _addBtn_" style="" onclick="javascript:location.href='/WAB002EmpCreate/init.do?tabId=<c:out value="${tabId}"></c:out>&menu=<c:out value="${menu}"></c:out>';" data-click-target="CLS">
			...
			<div class="WAB001ES-tbtnM pt5" style="display: inline-block;">
				...
				<button type="button" class="downLoad" onclick="javascript:getMaEmp();">
					<span  onclick="javascript:getMaEmp();">인사정보 받아오기</span>
				</button>			
			</div>
		...
		<c:if test="${fn:substring(authBinaryNum, 3, 4) eq '1' }"><%--조회권한 --%>
			<c:if test="${fn:length(empList) > 0}">
				<div class="pagenation">
		            <smpl:seqpaging maxSeq="10" action='${pageContext.request.contextPath}/WAB001EmpSearch/search.do?tabId=${tabId}&menu=${menu}' formId="WAB001EmpSearchForm" />
		        </div>
...
<script type="text/javascript">
function getMaEmp(){	
	var form = document.WAB001EmpSearchForm;
	form.maEmpYn.value = "Y";
	form.submit();
}
</script>
