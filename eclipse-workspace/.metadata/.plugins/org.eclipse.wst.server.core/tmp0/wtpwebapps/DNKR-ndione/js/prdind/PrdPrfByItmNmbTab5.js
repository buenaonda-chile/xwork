...
var cv5 = new wijmo.collections.CollectionView(null);	//컬랙션뷰 전역변수
var flexGrid5 = new wijmo.grid.FlexGrid('#wijmoGrid5');	//그리드 전역변수

function search5(){
	//validation 체크기능으로 입력칸에 커서가 활성화 되어있을경우 조회시 오류가 발생함.
	//validation edit 기능을 비활성화 시키고 조회시킴.
	flexGrid5.validateEdits = false;
	if(!$("#getComps").val()){
	...
	var url = "/ajaxPrdPrfByItmNmb/search5.do";	
	var params = {
 ...
 * 엑셀 다운로드
 */
function downloadExcel5(){
	
	var fileName = "직접생산투입시간_"+getToday()+".xlsx";
	
	wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync(flexGrid5, 
...
//	var comsBase = new DataMap(coms_base, 'id', 'name');
	...
	//컬럼 초기화
	flexGrid5.initialize({
		columns: columnsDefinition,
		allowMerging: 'Cells'
	});
	flexGrid5.allowSorting = false;		
//	flexGrid5.columnHeaders.rows.defaultSize = 80; 	// 열 헤더 영역에 행의 높이 설정		
}
