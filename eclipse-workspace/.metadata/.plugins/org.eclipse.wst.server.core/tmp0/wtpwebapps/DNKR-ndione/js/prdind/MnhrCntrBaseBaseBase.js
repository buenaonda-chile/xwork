...
function validation(item){	
	...
	if(item.cycleTime==null || wijmo.isEmpty(item.cycleTime)){
	}else {
		if(item.cycleTime.toString().split(".")[0].length > 4 ) {
			toastMsg("정수 값은 4자리를 초과하면 안됩니다.");
			return false;
		}
	}
	if(item.drctHdcn==null || wijmo.isEmpty(item.drctHdcn)){
	}else {
		if(item.drctHdcn.toString().split(".")[0].length > 6 ) {
			toastMsg("정수 값은 6자리를 초과하면 안됩니다.");
			return false;
		}
	}
	if(item.indrHdcn==null || wijmo.isEmpty(item.indrHdcn)){
	}else {
		if(item.indrHdcn.toString().split(".")[0].length > 6 ) {
			toastMsg("정수 값은 6자리를 초과하면 안됩니다.");
			return false;
		}
	}	
	if(item.jmLabor==null || wijmo.isEmpty(item.jmLabor)){
	}else {
		if(item.jmLabor.toString().split(".")[0].length > 4 ) {
			toastMsg("정수 값은 4자리를 초과하면 안됩니다.");
			return false;
		}
	}
	if(item.prfrTime==null || wijmo.isEmpty(item.prfrTime)){
	}else {
		if(item.prfrTime.toString().split(".")[0].length > 4 ) {
			toastMsg("정수 값은 4자리를 초과하면 안됩니다.");
			return false;
		}
	}
	
	if(item.stdrLabor==null || wijmo.isEmpty(item.stdrLabor)){
	}else {
		if(item.stdrLabor.toString().split(".")[0].length > 4 ) {
			toastMsg("정수 값은 4자리를 초과하면 안됩니다.");
			return false;
		}
	}
	if(item.excTmRt==null || wijmo.isEmpty(item.excTmRt)){
	}else {
		if(item.excTmRt.toString().split(".")[0].length > 4 ) {
			toastMsg("정수 값은 4자리를 초과하면 안됩니다.");
			return false;
		}
	}
	...
	if(item.indTmRt==null || wijmo.isEmpty(item.indTmRt)){
	}else {
		if(item.indTmRt.toString().split(".")[0].length > 4 ) {
			toastMsg("정수 값은 4자리를 초과하면 안됩니다.");
			return false;
		}
	}
	if(item.excIndRt==null || wijmo.isEmpty(item.excIndRt)){
	}else {
		if(item.excIndRt.toString().split(".")[0].length > 4 ) {
			toastMsg("정수 값은 4자리를 초과하면 안됩니다.");
			return false;
		}
	}					
...
var inputNumber4 = new wijmo.input.InputNumber(document.createElement('div'),{
	format:'n4'
});
...
//	var comsBase = new DataMap(coms_base, 'id', 'name');
	var columnsDefinition=[
		...
		{binding:'lineCode',header: '* '+ LB_LINE_CODE,width: 120, align:'center',dataType:'String', maxLength:4},
		{binding:'lineNm',header: '* '+ LB_LINE_NM, width: 130, align:'left',dataType:'String', maxLength:50,isReadOnly: true},
		{binding:'prdtNo',header: '* '+ LB_ITEM_CODE, width: 150, align:'center',dataType:'String', maxLength:50},		
		{binding:'prdtNm',header: '* '+ LB_ITEM_NM, width: 230, align:'left',dataType:'String', maxLength:50,isReadOnly: true},				
		{binding:'cycleTime',header: '* '+ LB_CYCLE_TIME, width: 110, align:'right',dataType:'Number', format: 'n4',editor:inputNumber4},	//C/T(sec)					
		{binding:'drctHdcn',header: '* '+ LB_DRCT_HDCN, width: 110, align:'right',dataType:'Number', format: 'n2', editor:inputNumber2},	//제조직접인원수(명)					
		{binding:'indrHdcn',header: '* '+ LB_INDR_HDCN, width: 110, align:'right',dataType:'Number', format: 'n2', editor:inputNumber2},	//제조간접인원수(조장,피다,검사)				
		{binding:'jmLabor',header: '* '+ LB_JM_LABOR, width: 110, align:'right',dataType:'Number', format: 'n4', editor:inputNumber4},		//정미공수								
		{binding:'prfrTime',header: '* '+ LB_PRFR_TIME, width: 110, align:'right',dataType:'Number', format: 'n4', editor:inputNumber4},	//실적시간													
		{binding:'mrgFct',header: '* '+ LB_MRG_FCT, width: 100, align:'right',dataType:'Number', isReadOnly: true},		//여유계수(생산실적시간/정미시간)*100			
		{binding:'stdrLabor',header: '* '+ LB_STDR_LABOR, width: 110, align:'right',dataType:'Number', format: 'n4', editor:inputNumber4},	//생산성기준공수(sec)=정미공수×여유계수
		{binding:'excTmRt',header: '* '+ LB_EXC_TM_RT, width: 110, align:'right',dataType:'Number', format: 'n4', editor:inputNumber4},	//제외시간율	
		{binding:'indTmRt',header: '* '+ LB_IND_TM_RT, width: 110, align:'right',dataType:'Number', format: 'n4', editor:inputNumber4},	//간접시간율				
		{binding:'excIndRt',header: '* '+ LB_EXC_IND_RT, width: 110, align:'right',dataType:'Number', format: 'n4',editor:inputNumber4},	//제외간접율							
		{binding:'sgmRgsArb',header: '* '+ LB_SGM_RGS_ARB, width: 100, align:'right',dataType:'Number', isReadOnly: true}, //시그마등록공수(S)															
		{binding:'uph',header: '* '+ LB_UPH, width: 100, align:'right',dataType:'Number', isReadOnly: true},				 //UPH	
	...
	cv.getError = function(item,prop){
		switch(prop){
			...
			case "prdtNo":
				...
				return null;
			case "cycleTime":
				if(item.cycleTime==null || wijmo.isEmpty(item.cycleTime) || item.cycleTime == ""){
				}else {
					return item.cycleTime.toString().split(".")[0].length > 4 ? "정수 값은 4자리를 초과하면 안됩니다" : "";					
				}
				return null;				
			case "drctHdcn":
				if(item.drctHdcn==null || wijmo.isEmpty(item.drctHdcn) || item.drctHdcn == ""){
				}else {
					return item.drctHdcn.toString().split(".")[0].length > 6 ? "정수 값은 6자리를 초과하면 안됩니다" : "";					
				}
				return null;					
			case "indrHdcn":
				if(item.indrHdcn==null || wijmo.isEmpty(item.indrHdcn) || item.indrHdcn == ""){
				}else {
					return item.indrHdcn.toString().split(".")[0].length > 6 ? "정수 값은 6자리를 초과하면 안됩니다" : "";					
				}
				return null;
			case "jmLabor":
				if(item.jmLabor==null || wijmo.isEmpty(item.jmLabor) || item.jmLabor == ""){
				}else {
					return item.jmLabor.toString().split(".")[0].length > 4 ? "정수 값은 4자리를 초과하면 안됩니다" : "";					
				}
				return null;
			case "prfrTime":
				if(item.prfrTime==null || wijmo.isEmpty(item.prfrTime) || item.prfrTime == ""){
				}else {
					return item.prfrTime.toString().split(".")[0].length > 4 ? "정수 값은 4자리를 초과하면 안됩니다" : "";					
				}
				return null;
				
			case "stdrLabor":
				if(item.stdrLabor==null || wijmo.isEmpty(item.stdrLabor) || item.stdrLabor == ""){
				}else {
					return item.stdrLabor.toString().split(".")[0].length > 4 ? "정수 값은 4자리를 초과하면 안됩니다" : "";					
				}
				return null;
			case "excTmRt":
				if(item.excTmRt==null || wijmo.isEmpty(item.excTmRt) || item.excTmRt == ""){
				}else {
					return item.excTmRt.toString().split(".")[0].length > 4 ? "정수 값은 4자리를 초과하면 안됩니다" : "";					
				}
				return null;				
				
			case "indTmRt":
				if(item.indTmRt==null || wijmo.isEmpty(item.indTmRt) || item.indTmRt == ""){
				}else {
					return item.indTmRt.toString().split(".")[0].length > 4 ? "정수 값은 4자리를 초과하면 안됩니다" : "";					
				}
				return null;
			case "excIndRt":
				if(item.excIndRt==null || wijmo.isEmpty(item.excIndRt) || item.excIndRt == ""){
				}else {
					return item.excIndRt.toString().split(".")[0].length > 4 ? "정수 값은 4자리를 초과하면 안됩니다" : "";					
				}
				return null;
		}
		return null;
