...
function validation(item){
	...
	if(item.clsCode==null || wijmo.isEmpty(item.clsCode) || item.clsCode == ""){
		...
		return false;
	}else if(item.locCode==null || wijmo.isEmpty(item.locCode) || item.locCode == ""){
		toastMsg("수정이나 추가한 행의 [반코드]컬럼을 입력해야 됩니다.");
		return false;
	}else{
	...
	if(item.mailAdr1==null || wijmo.isEmpty(item.mailAdr1) || item.mailAdr1 == "") {
		toastMsg("하나 이상의 이메일 주소를 입력해야 합니다.");
		return false;
	}
...
//		toastMsg(LN_SELECT_COMPS);
	...
	var url = "/ajaxApplnMailSetting/search.do";	
	var params = {
...
//	} 
    ...
    for ( var i = 0; i <cv.itemsEdited.length; i ++) {
		...
		$.ajax({
			method : "post",
			url : "/ajaxApplnMailSetting/update.do",
    ...
    for ( var i = 0; i <cv.itemsAdded.length; i ++) {
		...
		$.ajax({
			method : "post",
			url : "/ajaxApplnMailSetting/create.do",
    ...
    for ( var i = 0; i <cv.itemsRemoved.length; i ++) {
		...
		$.ajax({
			method : "post",
			url : "/ajaxApplnMailSetting/delete.do",
...
function downloadExcel(){
	var fileName = "생산성지표메일설정_"+getToday()+".xlsx";
...
function gridInit(){
	var columnsDefinition=[
		...
		{header:'반', align: 'center', columns: [
			{binding:'locCode',header: LB_LOC_CODE ,width: 110, align:'center',dataType:'String', isReadOnly: false},
			{binding:'locNm',header: LB_LOC_NM ,width: 150, align:'center',dataType:'String', isReadOnly: true}
		]},
		{header:'승인1', align: 'center', columns: [
			{binding:'aprvlBy1',header: '성명', width: 95, align:'center',dataType:'String', maxLength:10},
			{binding:'mailAdr1',header: '메일주소', width: 190, align:'center',dataType:'String',  maxLength:30}
		]},
		{header:'승인2', align: 'center', columns: [
			{binding:'aprvlBy2',header: '성명', width: 95, align:'center',dataType:'String', maxLength:10},
			{binding:'mailAdr2',header: '메일주소', width: 190, align:'center',dataType:'String',  maxLength:30}
		]},
		{header:'승인3', align: 'center', columns: [
			{binding:'aprvlBy3',header: '성명', width: 95, align:'center',dataType:'String', maxLength:10},
			{binding:'mailAdr3',header: '메일주소', width: 190, align:'center',dataType:'String',  maxLength:30}
		]},
		{header:'승인4', align: 'center', columns: [
			{binding:'aprvlBy4',header: '성명', width: 95, align:'center',dataType:'String', maxLength:10},
			{binding:'mailAdr4',header: '메일주소', width: 190, align:'center',dataType:'String',  maxLength:30}
		]},
		{header:'승인5', align: 'center', columns: [
			{binding:'aprvlBy5',header: '성명', width: 95, align:'center',dataType:'String', maxLength:10},
			{binding:'mailAdr5',header: '메일주소', width: 190, align:'center',dataType:'String',  maxLength:30}
		]},
