...
var cv7 = new wijmo.collections.CollectionView(null);	//컬랙션뷰 전역변수
var flexGrid7 = new wijmo.grid.FlexGrid('#wijmoGrid7');	//그리드 전역변수

function search7(){
	//validation 체크기능으로 입력칸에 커서가 활성화 되어있을경우 조회시 오류가 발생함.
	//validation edit 기능을 비활성화 시키고 조회시킴.
	flexGrid7.validateEdits = false;
	if(!$("#getComps").val()){
	...
	var url = "/ajaxPrdPrfByItmNmb/search7.do";	
	var params = {
...
function downloadExcel7(){
	...
	var fileName = "설총율_"+getToday()+".xlsx";
	
	wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync(flexGrid7, 
...
//		{binding:'ttl',header: LB_TTL, width: 100,align:'right',dataType:'Number',isReadOnly: true}					
	...
	//컬럼 초기화
	flexGrid7.initialize({
		columns: columnsDefinition,
		allowMerging: 'Cells'
	});
	flexGrid7.allowSorting = false;			
//	flexGrid7.columnHeaders.rows.defaultSize = 80; 	// 열 헤더 영역에 행의 높이 설정		
}
