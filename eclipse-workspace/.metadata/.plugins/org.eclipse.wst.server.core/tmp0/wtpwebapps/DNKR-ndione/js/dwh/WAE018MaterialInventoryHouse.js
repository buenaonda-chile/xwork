...
function search() {
	...
	var pGubun = $('#gubun').val();	//구분
	var pHouse = $('#house').val();	//창고
	var pPrdcd = $('#prdcd').val();	//제품코드
	var pPlanner = $('#plann').val();	//planner
	//var pLocCode = $('#locCode').val();	//사업부
	var pLocCode ="LOC000001";	//사업부(창원 강제)
	
	if (val_chk(pGubun, "구분")) {
		var params = {gubun: pGubun, house: pHouse, prdcd: pPrdcd, plann: pPlanner, locCode: pLocCode};
		var objList = getAjaxList("/WAE018MaterialInventoryHouseSearch/ajaxSearch.do", params)
	}
	return false;
}
...
function createGrid(){
	unitGrid = new ax5.ui.grid({
        ...
        columns: [
            ...
            {key: "recmo", label: "입고", formatter: "money", align: "right"},
            {key: "issmo", label: "출고", formatter: "money", align: "right"},          
