...
var cv2 = new wijmo.collections.CollectionView(null);	//컬랙션뷰 전역변수
...
function search2(){
	...
	var url = "/ajaxPrdPrfByItmNmb/search2.do";	
	var params = {
...
function downloadExcel2(){
	...
	var fileName = "생산실적_"+getToday()+".xlsx";
	
	wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync(flexGrid2, 
...
//	var comsBase = new DataMap(coms_base, 'id', 'name');
	var columnsDefinition=[
		...
		{binding:'ttl',header: '누계', width: 100,align:'right',dataType:'Number',isReadOnly: true}																			
	];	
	//컬럼 초기화
...
//	flexGrid2.columnHeaders.rows.defaultSize = 80; 	// 열 헤더 영역에 행의 높이 설정
	
	
//	var flexGridFilter = new wijmo.grid.filter.FlexGridFilter(flexGrid2);
//	flexGridFilter.filterColumns = ['lineCode'];
	flexGrid2.allowSorting = false;
	//필터제거
	//flexGridFilter.filterColumns = [];	
	
//	flexGrid2.allowPinning = "SingleColumn";
