...
function paymentRequest() {
	...
	var url = "/ajaxStEmpAbsCnf/paymentRequest.do";	
	var params = {
	...
	$.ajax({
		...
		success : function(xhr) {
			...
			if(data.result == RESULT_OK) {
				toastMsg("결제가 요청되었습니다.");
				window.open(data.url,"paymentRequestPop","width=550,height=600,scrollbars=yes,top=100,left=100");
			}
		},
		error : function() {
...
function search(){
	...
	var url = "/ajaxStEmpAbsCnf/search.do";	
	var params = {
		'getComps' : $("#getComps").val()
		,'getLocId' : $("#getLocId").val()
		,'getOpertDe' : $("#getOpertDe").val()		
...
//			typeof cv._src[i].startTime != "undefined" && cv._src[i].startTime != "" && cv._src[i].startTime != null &&
//			typeof cv._src[i].endTime != "undefined" && cv._src[i].endTime != "" && cv._src[i].endTime != null 		
		){		
			cv._src[i].rownum = i;
			$.ajax({
				method : "post",
				url : "/ajaxStEmpAbsCnf/update.do",
				data : cv._src[i],
				async: false,
				success : function(xhr) {
	    	    	var data = JSON.parse(xhr);
...
function rcvsupTmUpdate() {
	...
	$.ajax({
		method : "post",
		url : "/ajaxStEmpAbsCnf/SaveRcvsupTm.do",
		data : paramsData,
		async: false,
		success : function(xhr) {
        	var data = JSON.parse(xhr);
...
//	var comsBase = new DataMap(coms_base, 'id', 'name');
	var columnsDefinition=[	
		...
		{binding:'shiftwork',visible:false},
		...
		{binding:'shiftworkNm',header: '근무조', width: '*', align:'center',dataType:'String',  maxLength:50,isReadOnly: true},
		{binding:'startTime',header: '출근', width: 150, align:'center',dataType:'String',isReadOnly: true},		
		{binding:'endTime',header: '퇴근', width: 150, align:'center',dataType:'String',isReadOnly: true},	
...
//		]},	
		{binding:'arqstTime',header: '지원보낸시간', width: '*', align:'center',dataType:'Number', editor:inputNumber, isRequired:false},
		{binding:'logiType',header: '물류Check', width: '*', align:'center',dataType:'Boolean', maxLength:1},		
		{binding:'extnBftm',header: '연장(조)', width: '*', align:'center',   dataType:'Number', editor:inputNumber, isRequired:false},
		{binding:'extnAftm',header: '연장(석)', width: '*', align:'center',   dataType:'Number', editor:inputNumber, isRequired:false},
		{binding:'extnTm',header: '연장(기타)', width: '*', align:'center',   dataType:'Number', editor:inputNumber, isRequired:false},
		...
		{binding:'workPsbTime',header: '근무가능시간', width: '*', align:'center',dataType:'Number',editor:inputNumber, isReadOnly: true},
		{binding:'workStle',header: '신청결과', width: '*', align:'center',dataType:'String', isReadOnly: true},	
		{binding:'saveSts',header: '저장상태', width: '*', align:'center',dataType:'String', isReadOnly: true},		
