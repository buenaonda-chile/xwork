...
var comboStatus = new wijmo.input.ComboBox("#status");	//발주진행상태

var sdat = new wijmo.input.InputDate("#searchOrdDtFr");	//발주일자 시작
		
var edat = new wijmo.input.InputDate("#searchOrdDtTo");	//발주일자 종료

var dat = new wijmo.input.InputDate(document.createElement('div')); //발주일자
...
var handleEvent = function(){
	...
	$("#btn_add").on("click", function() {
		...
		var date = new Date();
		addRow.orderDt = getFormatDate(date);
		addRow.orderNo = "[발주번호는 자동생성됩니다.]";
		addRow.status = "O1";	//발주상태 기본 "발주"로
...
var validation = function(item){
	...
	if (item.orderDt == null || wijmo.isEmpty(item.orderDt) || item.orderDt == "") {
		alert("[발주일자를 입력 해주세요.]");
		return false;
	}
...
var save = function(){
	...
	for ( var i = 0; i <cv.itemsAdded.length; i ++) {
		...
		ajaxCommit("/orderCDRepairReplacement/create",cv.itemsAdded[i]);
	...
	for ( var i = 0; i <cv.itemsEdited.length; i ++) {
		...
		ajaxCommit("/orderCDRepairReplacement/update",cv.itemsEdited[i]);
	}
	...
	for ( var i = 0; i <cv.itemsRemoved.length; i ++) {
		...
		ajaxCommit("/orderCDRepairReplacement/delete",cv.itemsRemoved[i]);
...
var search = function(){
	...
	var url = '/orderCDRepairReplacement/search';
...
var gridInit = function() {
	...
	columnsDefinition = [
	    ...
	    {binding: 'orderNo', header: '발주번호', width: '*', align:'center',dataType:'Number',isReadOnly:true},  
        {binding: 'orderDt', header: '발주일자', width: '*', align:'center',dataType:'String',editor: dat}, //wordWrap: true: 텍스트 자동줄바꿈 autorowHeights랑 같이 사용
        {binding: 'partNo', header: '수리대체품번호', width: '*', align:'center',dataType:'String',isReadOnly:true},
		{binding: 'partNm', header: '수리대체품명', width: '*', align:'center',dataType:'String',isReadOnly:true},
        ...
        {binding: 'custNm', header: '발주처', width: '*', align:'center',dataType:'String',isReadOnly:true},
  		{binding: 'custId', header: '발주처아이디', width: '*', align:'center',dataType:'Number',visible:false,isReadOnly:true},
		{header: '', width: 40, align:'center',
					cellTemplate: cellMaker.makeButton({
								text:"<img src='/assets/img/lib/ic_search1.png'>",
								click: function(e, ctx) {
        ...
        {binding: 'custEmpNm', header: '발주처담당자', width: '*', align:'center',dataType:'String',isReadOnly:true},
		{binding: 'custEmpId', header: '담당자아이디', width: '*', align:'center',dataType:'Number',isReadOnly:true,visible:false},
        {binding: 'status', header: '진행상태', width: '*', dataMap:statusMap, align:'center',dataType:'String',isReadOnly:true},
...
var statusInit = function(){
	...
	var url = '/code/list2.json/ORDER_STATUS';
