...
var sdat = new wijmo.input.InputDate("#searchRecDtFr");	//발주일자 시작
		
var edat = new wijmo.input.InputDate("#searchRecDtTo");	//발주일자 종료
...
var handleEvent = function(){
	...
	$("#btn_add").on("click", function() {
		...
		var date = new Date();
		addRow.rectDt = getFormatDate(date);
		addRow.rectNo = "[접수번호는 자동생성됩니다.]";
		addRow.status = "R1";	//접수상태 기본 "발주"로
...
var validation = function(item){
	...
	if (item.rectDt == null || wijmo.isEmpty(item.rectDt) || item.rectDt == "") {
		alert("[접수일자를 입력 해주세요.]");
		return false;
	}
...
var save = function(){
	...
	for ( var i = 0; i <cv.itemsAdded.length; i ++) {
		...
		ajaxCommit("/receiptCDRepairReplacement/create",cv.itemsAdded[i]);
	...
	for ( var i = 0; i <cv.itemsEdited.length; i ++) {
		...
		ajaxCommit("/receiptCDRepairReplacement/update",cv.itemsEdited[i]);
	}
	...
	for ( var i = 0; i <cv.itemsRemoved.length; i ++) {
		...
		ajaxCommit("/receiptCDRepairReplacement/delete",cv.itemsRemoved[i]);
...
var search = function(){
	...
	var url = '/receiptCDRepairReplacement/search';
...
var gridInit = function() {
	...
	columnsDefinition = [
        ...
        {binding: 'rectDt', header: '접수일자', width: '*', align:'center',dataType:'String',editor: dat}, //wordWrap: true: 텍스트 자동줄바꿈 autorowHeights랑 같이 사용
        ...
        {binding: 'custNm', header: '접수처', width: '*', align:'center',dataType:'String',isReadOnly:true},
  		{binding: 'custId', header: '접수처아이디', width: '*', align:'center',dataType:'Number',visible:false,isReadOnly:true},
		{header: '', width: 40, align:'center',
					cellTemplate: cellMaker.makeButton({
								text:"<img src='/assets/img/lib/ic_search1.png'>",
								click: function(e, ctx) {
...
var gridEvent = function(){
	...
	cv.getError = function(item, prop) {
		...
		switch (prop) {
			case "rectDt":
				if (item.rectDt == null || wijmo.isEmpty(item.rectDt) || item.rectDt == "") {
					return "[접수일자를 입력 해주세요.]";
				}
				return null;
				
			case "partNo":
				if (item.partNo == null || wijmo.isEmpty(item.partNo) || item.partNo == "") {
					return "[수리부품번호를 입력 해주세요.]";
				}
				return null;
